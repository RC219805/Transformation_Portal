...
class DA3InferenceEngine:
    ...
    def infer(
        ...
        **kwargs
        ...
        """
        if self.use_cli:
            return self._infer_cli(images, export_dir, convert_to_metric, 
                                   focal_length_px, fov_degrees, **kwargs)
        else:
            return self._infer_api(images, extrinsics, intrinsics, export_dir,
                                   convert_to_metric, focal_length_px, fov_degrees, **kwargs)
    ...
    def _infer_api(
        ...
        **kwargs
        ...
        if convert_to_metric:
            ...
            logger.info(
                f"Converted to metric depth: "
                f"{metric_result.scale_factor:.4f}x scale factor"
            )
    ...
    def _infer_cli(
        ...
        **kwargs
        ...
        if len(images) == 1:
            result = self.cli.process_image(
                image_path=images[0],
                export_dir=export_dir or Path("output"),
                **kwargs
            )
        else:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                result = self.cli.process_images(
                    images_dir=tmppath,
                    export_dir=export_dir or Path("output"),
                    **kwargs
                )
    ...
    def load_model(self, force_reload: bool = False):
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install depth-anything-v3"
            )
