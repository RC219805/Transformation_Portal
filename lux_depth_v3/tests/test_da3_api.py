...
class TestDepthAnything3Wrapper:
    ...
    def test_wrapper_initialization_no_api(self):
        ...
        with patch('lux_depth_v3.da3_wrapper.DepthAnything3Wrapper.__init__',
                   return_value=None):
    ...
    @patch('lux_depth_v3.da3_wrapper.DepthAnything3Wrapper.__init__',
           return_value=None)
    ...
    def test_inference_ref_view_validation(self, mock_load):
        ...
        with pytest.raises(ValueError, match="Invalid ref_view_strategy"):
            wrapper.inference(
                image=["test.jpg"],
                ref_view_strategy="invalid_strategy"
            )
    ...
    def test_from_pretrained(self):
        ...
        with patch.object(DepthAnything3Wrapper, '__init__',
                         return_value=None) as mock_init:
            wrapper = DepthAnything3Wrapper.from_pretrained(
...
class TestInferenceEngineAPI:
    ...
    def test_engine_init_with_api(self, mock_wrapper_class):
        ...
        config = DA3Config(
            api=DA3APIConfig(model_name="da3-large")
        )
    ...
    def test_engine_infer_api(self, mock_wrapper_class):
        ...
        mock_prediction = DA3Prediction(
            depth=np.random.rand(1, 100, 100),
            conf=None
        )
