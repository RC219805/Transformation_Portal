...
class MetricDepthConverter:
    ...
    def convert(
        ...
        logger.info(
            f"Converted {self.model_name} depth to meters "
            f"(focal={focal:.2f}px, scale={scale_factor:.4f})"
        )
    ...
    def _extract_focal_from_intrinsics(
        self,
        ...
        if intrinsics.ndim == 2 and intrinsics.shape == (3, 3):
            ...
            fy = intrinsics[0, 1, 1]
            logger.warning(
                f"Multiple intrinsics provided, using first camera "
                f"(fx={fx:.2f}, fy={fy:.2f})"
            )
            return float((fx + fy) / 2.0)
