...
class RefViewSelectionResult:
    ...
    def __str__(self) -> str:
        return (
            f"Selected view {self.selected_index} "
...
class ReferenceViewSelector:
    ...
    def _select_saddle_balanced(
        ...
        normalized_tokens = class_tokens / (
            np.linalg.norm(class_tokens, axis=-1, keepdims=True) + 1e-8
        )
        ...
        distances_to_median = (
            np.abs(norm_similarity - 0.5) +
            np.abs(norm_norms - 0.5) +
            np.abs(norm_variances - 0.5)
        )
