...
class TSDFFusion:
    ...
    def integrate(self, depth: np.ndarray, rgb: np.ndarray, intrinsics: np.ndarray, extrinsics: np.ndarray) -> None:
        ...
        self.volume.integrate(rgbd, intrinsic_o3d, extrinsics)
...
def fuse_depth_maps(
    ...
    **fusion_kwargs,
    ...
    # Initialize TSDF volume
    fusion = TSDFFusion(voxel_length=voxel_length, sdf_trunc=sdf_trunc, **fusion_kwargs)

    # Integrate all frames
    logger.info(f"Fusing {len(depth_maps)} depth maps...")

    for i, (depth, rgb, K, extr) in enumerate(zip(depth_maps, rgb_images, intrinsics, extrinsics)):
        try:
    ...
    logger.info(f"Fusion complete: {len(mesh.vertices)} vertices, {len(mesh.triangles)} triangles")
...
def fuse_depth_maps_multiprocess(
    ...
    **fusion_kwargs,
    ...
    logger.info(f"Multi-process fusion requested with {num_workers} workers, but using sequential fusion (Open3D limitation)")

    return fuse_depth_maps(depth_maps, rgb_images, intrinsics, extrinsics, **fusion_kwargs)
