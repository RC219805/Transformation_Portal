...
def evaluate_pose_estimation(
    ...
    if len(pred_poses) != len(gt_poses):
        raise ValueError(
            f"Number of predicted poses ({len(pred_poses)}) "
            f"doesn't match GT poses ({len(gt_poses)})"
        )
...
def compute_pose_metrics_batch(
    ...
    if scene_names is None:
        ...
        return {
            "overall": evaluate_pose_estimation(
                list(pred_poses), list(gt_poses)
            )
        }
    ...
    # Compute overall metrics
    results["overall"] = evaluate_pose_estimation(
        list(pred_poses), list(gt_poses)
    )
