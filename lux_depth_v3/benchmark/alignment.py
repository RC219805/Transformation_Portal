...
def align_poses_ransac(
    ...
    for _ in range(ransac_iterations):
        ...
        if len(pred_positions) < 3:
            ...
            sample_indices = np.random.choice(
                len(pred_positions), size=3, replace=False
            )
    ...
    if best_inliers > 3:
        ...
        if np.sum(inliers) > 3:
            best_rotation, best_translation = compute_rigid_transform(
                pred_positions[inliers],
                gt_positions[inliers]
            )
    
    inlier_ratio = best_inliers / len(pred_poses)
    logger.info(
        f"RANSAC alignment: {best_inliers}/{len(pred_poses)} inliers "
        f"({inlier_ratio:.1%})"
    )
