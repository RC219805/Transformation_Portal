...
class DA3BenchmarkEvaluator:
    ...
    def __init__(
        ...
        if not use_cli:
            if not HAS_INFERENCE:
                raise ImportError(
                    ...
                    "Use use_cli=True to use DA3 CLI instead."
                )
        ...
        for dataset_name in self.config.datasets:
            try:
                self.datasets[dataset_name] = get_dataset(
    ...
    def run_inference(self, dataset_name: str, scene_name: str) -> Dict:
        ...
        logger.info(
            f"Running inference on {dataset_name}/{scene_name} "
    ...
    def _run_inference_cli(
        ...
        # Run CLI
        cmd = [
            "python", "-m", "lux_depth_v3.cli",
            "batch",
            "--input-list", str(input_list),
            "--output-dir", str(output_dir / "depth"),
            "--format", "npy"
        ]
    ...
    def evaluate_pose(self, dataset_name: str, scene_name: str) -> Dict:
        ...
        if not depth_dir.exists():
            raise FileNotFoundError(
                f"Depth predictions not found: {depth_dir}. "
                "Run inference first."
            )
        ...
        # Compute metrics
        metrics = evaluate_pose_estimation(
            list(pred_poses_aligned), list(gt_poses)
        )
                    ...
                    for mode in self.config.modes:
                        if mode == EvaluationMode.POSE:
                            ...
                            metrics = self.evaluate_reconstruction(
                                dataset_name, scene_name, use_gt_poses=False
                            )
                            scene_results["recon_unposed"] = metrics
                        
                        elif mode == EvaluationMode.RECON_POSED:
                            metrics = self.evaluate_reconstruction(
                                dataset_name, scene_name, use_gt_poses=True
                            )
                    ...
                    if mode in EXPECTED_RESULTS[dataset_name]:
                        ...
                        for key, exp_val in expected.items():
                            if key in avg_metrics:
                                ...
                                print(
                                    f"    {key}: {exp_val:.4f} "
                                    f"(actual: {act_val:.4f}, diff: {diff:+.4f}) {status}"
                                )
