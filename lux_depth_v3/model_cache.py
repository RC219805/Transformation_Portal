...
class ModelCacheManager:
    ...
    def download_model(
        ...
        if model_key not in self.OFFICIAL_MODELS:
            raise ValueError(
                ...
                f"Available: {list(self.OFFICIAL_MODELS.keys())}"
            )
    ...
    def _download_hf_cache(self, model_id: str) -> ModelCacheInfo:
        ...
        try:
            ...
            raise RuntimeError(
                "depth_anything_3 package not installed. "
    ...
    def download_models(
        ...
        if model_keys:
            ...
            raise ValueError(
                f"Unknown model set: {model_set}. "
                f"Available: {list(self.RECOMMENDED_SETS.keys())}"
            )
        ...
        for key in keys:
            try:
                ...
                results.append(info)
                logger.info(
                    f"✓ {key}: {info.size_bytes / (1024**3):.2f} GB"
                )
