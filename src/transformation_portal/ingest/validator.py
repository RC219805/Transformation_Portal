...
def validate_schema(
    ...
    3. Type validation
    4. Unknown fields detection (if strict_mode=True)
    
    Args:
        data: Dictionary, JSON string, or Path to JSON file
        schema_type: "provenance" or "manifest"
        strict_mode: If True, fail on unknown fields (drift detection)
    ...
    if not schema_version:
        ...
        errors.append(
            ...
            f"This validator supports version 1.0.0 only."
        )
    
    # Detect unknown fields (drift)
    if strict_mode:
        unknown_fields = _detect_unknown_fields(data_dict, schema_class)
        if unknown_fields:
            errors.append(
                f"Unknown fields detected (schema drift): {', '.join(unknown_fields)}"
            )
    ...
    try:
        ...
        for error in e.errors():
            ...
            if error_type == "value_error.missing":
                ...
                errors.append(
                    f"Type mismatch at {field_path}: {error_msg}"
                )
            else:
                errors.append(
    ...
    try:
        ...
        if expected_dtype in ["uint16", "float32", "float64"]:
            if actual_dtype in ["uint8", "int8"]:
                return (
                    f"8-bit conversion detected: expected {expected_dtype}, "
                    f"got {actual_dtype}"
                )
        ...
        if expected_dtype == "uint16":
            if image_data.max() <= 255:
                return (
                    "8-bit range detected in uint16 image: "
                    f"max value is {image_data.max()} (expected > 255 for 16-bit)"
                )
...
def validate_linear_gamma(
    ...
    try:
        ...
        hist, _ = np.histogram(normalized.flatten(), bins=10, range=(0, 1))
        ...
        if shadow_ratio < 0.2 - tolerance:
            return (
                f"Non-linear gamma detected: only {shadow_ratio:.1%} of pixels "
                f"in shadow range (expected ≥ 20% for linear)"
            )
