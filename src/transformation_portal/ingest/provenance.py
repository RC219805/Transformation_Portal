...
def _extract_exif_with_exiftool(file_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.returncode != 0:
            raise ProvenanceCaptureError(f"exiftool extraction failed for {file_path.name}: {result.stderr}")
        ...
        raise ProvenanceCaptureError(f"exiftool extraction timed out for {file_path.name}") from e
    except json.JSONDecodeError as e:
        raise ProvenanceCaptureError(f"Failed to parse exiftool JSON output for {file_path.name}") from e
