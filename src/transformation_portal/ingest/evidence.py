...
_ALLOWED_MACHINE_COMMANDS = frozenset(
    {
        "check-system",
        "extract",
        "validate",
...
def _decode_pointer_token(token: str) -> str:
    # RFC 6901 requires "~0" -> "~" before
    # "~1" -> "/" to avoid double-unescaping.
    return token.replace("~0", "~").replace("~1", "/")


def _split_json_pointer(pointer: str) -> tuple[str, ...]:
    if not isinstance(pointer, str) or not pointer.startswith("/"):
        raise ValueError("drop path must be an absolute" f" JSON pointer: {pointer!r}")
    return tuple(_decode_pointer_token(token) for token in pointer.split("/")[1:])


def _drop_path_in_place(
    node: Any,
    tokens: tuple[str, ...],
    index: int = 0,
) -> None:
...
def load_projection_profile(
    ...
    if path is None:
        ...
        try:
            ...
            raise ValueError("unable to load projection " f"profile {path}: {exc}") from exc

    if not isinstance(raw_profile, dict):
        raise ValueError("projection profile must be a JSON object")
    if raw_profile.get("schema") != DEFAULT_PROJECTION_PROFILE:
        raise ValueError("projection profile schema must be " f"{DEFAULT_PROJECTION_PROFILE}")
    if raw_profile.get("source_schema") != MACHINE_SCHEMA_VERSION:
        raise ValueError("projection profile source_schema " f"must be {MACHINE_SCHEMA_VERSION}")
...
def project_machine_envelope(
    ...
    *,
    ...
    if machine_payload.get("schema") != MACHINE_SCHEMA_VERSION:
        ...
        raise ValueError(f"machine payload schema must be " f"{MACHINE_SCHEMA_VERSION}, got {found!r}")
...
def _extract_file_sha256(projected_envelope: Mapping[str, Any]) -> str | None:
    ...
    if isinstance(file_integrity, Mapping) and "sha256" in file_integrity:
        try:
            return _validate_sha256(
                file_integrity.get("sha256"),
                field="file_integrity.sha256",
            )
        except ValueError:
            return None
    ...
    if isinstance(data, Mapping):
        ...
        if isinstance(data_file_integrity, Mapping) and "sha256" in data_file_integrity:
            try:
                return _validate_sha256(
                    data_file_integrity.get("sha256"),
                    field="data.file_integrity.sha256",
                )
            except ValueError:
                return None
    return None
...
def _validate_machine_contract_surface(
    machine_payload: Mapping[str, Any],
) -> tuple[str, bool, int]:
...
def build_evidence_payload(
    ...
    *,
    ...
    profile = copy.deepcopy(dict(projection_profile or load_projection_profile()))
    projected_envelope = project_machine_envelope(
        machine_payload,
    ...
    if bundle_root_sha256 is not None:
        bundle_root_sha256 = _validate_sha256(
            bundle_root_sha256,
            field="bundle_root_sha256",
        )
