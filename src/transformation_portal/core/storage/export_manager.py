...
from .autotune_helpers import compute_image_stats, ImageStats
...
class ExportManager:
    ...
    def _write_file(img: np.ndarray, path: Path, config: ExportConfig):
        ...
        if ext in ["jpg", "jpeg"]:
            cv2.imwrite(
                str(path), img_bgr, 
                [int(cv2.IMWRITE_JPEG_QUALITY), config.quality_jpg]
            )
            ...
            if img.dtype == np.uint16:
                cv2.imwrite(
                    str(path), img_bgr,
                    [int(cv2.IMWRITE_PNG_COMPRESSION), config.compression_png]
                )
            else:
                # Standard PNG
                cv2.imwrite(
