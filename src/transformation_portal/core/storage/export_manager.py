...
class ExportManager:
    ...
    def _validate_config(self) -> None:
        ...
        if self.config.tiff_tile_size is not None:
            ...
            if (
                ts < self.config.tiff_tile_size_min
                or ts > self.config.tiff_tile_size_max
            ):
                raise ValueError(
    ...
    def _write_tiff16(
        self, path: Path, arr: np.ndarray, compression: Optional[str] = "deflate"
    ) -> None:
        """
        ...
        effective_comp = (
            compression if compression is not None else self.config.tiff_compression
        )
        ...
        if self.config.tiff_tile_size is not None:
            ...
            write_tiff16_tiled(
                path, arr, tile_size=tile_size, compression=effective_comp
            )
        else:
    ...
    def _write_image_atomic(
        self, final_path: Path, arr: np.ndarray, compression: str = "deflate"
    ) -> None:
        """
    ...
    def write_master(
        self, stem: str, master_arr: np.ndarray, compression: str = "deflate"
    ) -> Path:
        """
    ...
    def write_upscaled(
        self, stem: str, upscaled_arr: np.ndarray, compression: str = "deflate"
    ) -> Path:
        """
        ...
        """
        filename = (
            f"{self.config.upscaled_prefix}{stem}{self.config.upscaled_suffix}.tif"
        )
        final_path = self.config.output_dir / filename
    ...
    def write_preview(
        self, stem: str, preview_arr: np.ndarray, quality: int = 92
    ) -> Path:
        """
        ...
        """
        filename = (
            f"{self.config.preview_prefix}{stem}{self.config.preview_jpg_suffix}.jpg"
        )
    ...
    def write_marketing_png(self, stem: str, png_arr: np.ndarray) -> Path:
        ...
        filename = (
            f"{self.config.upscaled_prefix}{stem}{self.config.marketing_suffix}.png"
        )
        path = self.config.output_dir / filename
        ...
        self._last_marketing_metadata = {
            ...
            "cpu_percent_delta": (
                round(cpu_end - cpu_start, 1) if cpu_available else None
            ),
        }
...
def autotune_export_config(
    ...
    marketing_cfg = MarketingExportConfig(
        png_compression_level=marketing_png_compression
    )
    ...
    # Compute megapixels
    megapixels = (
        (image_width * image_height) / 1_000_000
