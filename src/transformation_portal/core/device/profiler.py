...
class PerformanceProfiler:
    ...
    def __init__(
        self, enable_memory_tracking: bool = True, enable_gpu_profiling: bool = True
    ):
        """
        Initialize profiler.
        ...
        if self.enable_gpu_profiling and not (
    ...
    def profile(self, name: str, **metadata):
        ...
        gpu_memory_start = (
            self._get_gpu_memory_mb() if self.enable_gpu_profiling else None
        )
    ...
    def print_summary(self, top_n: Optional[int] = None):
        ...
        sorted_results = sorted(
            self._results, key=lambda r: r.duration_ms, reverse=True
        )
        ...
        for result in sorted_results:
            ...
            logger.info(
                f"{result.name:.<40} {result.duration_ms:>8.1f}ms ({pct:>5.1f}%)"
            )
            ...
            if result.memory_delta_mb is not None:
                logger.info(
                    f"  {'Memory change':<38} {result.memory_delta_mb:>+8.1f}MB"
                )

            if result.gpu_memory_delta_mb is not None:
                logger.info(
                    f"  {'GPU memory change':<38} {result.gpu_memory_delta_mb:>+8.1f}MB"
                )
...
class GPUProfiler:
    ...
    def profile(self, name: str):
        ...
        try:
            ...
            if self._cuda_available:
                ...
                self.events.append(
                    {"name": name, "cpu_ms": cpu_duration, "gpu_ms": gpu_duration}
                )
            else:
                self.events.append({"name": name, "cpu_ms": cpu_duration})
    ...
    def report(self) -> dict:
        ...
        return {"total_ms": total_time, "stages": self.events.copy()}
    ...
    def print_report(self):
        ...
        for event in report["stages"]:
            ...
            if "gpu_ms" in event:
                logger.info(
                    f"{name:.<40} GPU: {event['gpu_ms']:>7.1f}ms  CPU: {event['cpu_ms']:>7.1f}ms"
                )
            else:
