...
class DeviceConfig(BaseModel):
    ...
    precision: PrecisionType = Field(
        default=PrecisionType.FP16, description="Computation precision"
    )
    enable_cudnn_benchmark: bool = Field(
        default=True, description="Enable cuDNN benchmarking for CUDA"
    )
    memory_fraction: float = Field(
        default=0.85, ge=0.1, le=0.95, description="Fraction of device memory to use"
    )
    prefer_neural_engine: bool = Field(
        default=True, description="Prefer Apple Neural Engine when available"
    )
...
class PathsConfig(BaseModel):
    ...
    input_dir: Optional[Path] = Field(
        default=None, description="Input directory for batch processing"
    )
    output_dir: Optional[Path] = Field(
        default=None, description="Output directory for results"
    )
    cache_dir: Path = Field(
        default=Path(".cache"), description="Cache directory for intermediate files"
    )
    ...
    model_weights_dir: Optional[Path] = Field(
        default=None, description="Directory for model weights"
    )

    @field_validator(
        "input_dir", "output_dir", "cache_dir", "checkpoint_dir", "model_weights_dir"
    )
    @classmethod
...
class PerformanceConfig(BaseModel):
    ...
    tile_size: int = Field(
        default=512, ge=64, description="Tile size for memory-efficient processing"
    )
    tile_overlap: int = Field(
        default=64, ge=0, description="Overlap between tiles in pixels"
    )
    enable_tiling: bool = Field(
        default=True, description="Enable tiled processing for large images"
    )
    enable_async_io: bool = Field(
        default=True, description="Enable asynchronous I/O operations"
    )
    enable_caching: bool = Field(default=True, description="Enable result caching")
    memory_budget_gb: Optional[float] = Field(
        default=None, ge=1.0, description="Memory budget in GB (None = auto)"
    )
    ...
    def validate_tile_overlap(cls, v: int, info) -> int:
        ...
        if v >= tile_size:
            raise ValueError(
                f"tile_overlap ({v}) must be less than tile_size ({tile_size})"
            )
        return v
...
class OutputConfig(BaseModel):
    ...
    save_master: bool = Field(
        default=True, description="Save master output (16-bit TIFF)"
    )
    save_preview: bool = Field(default=True, description="Save preview (JPEG/PNG)")
    preview_scale: float = Field(
        default=0.25, ge=0.01, le=1.0, description="Preview scale factor"
    )
    compression: Optional[str] = Field(
        default="lzw", description="TIFF compression (lzw, deflate, none)"
    )
    skip_existing: bool = Field(
        default=True, description="Skip processing if output exists"
    )
    overwrite: bool = Field(default=False, description="Overwrite existing outputs")
    write_outputs: bool = Field(
        default=True, description="Master switch for filesystem output"
    )
...
class ValidationConfig(BaseModel):
    ...
    validate_ai_output: bool = Field(
        default=True, description="Validate AI-generated outputs"
    )
    max_input_size_mb: float = Field(
        default=500.0, ge=1.0, description="Maximum input file size in MB"
    )
    ...
    strict_mode: bool = Field(
        default=False, description="Strict validation mode (fail on warnings)"
    )
...
class ConfigSchema(BaseModel):
    ...
    device: DeviceConfig = Field(
        default_factory=DeviceConfig, description="Device configuration"
    )
    ...
    extras: Dict[str, Any] = Field(
        default_factory=dict, description="Pipeline-specific configuration extensions"
    )
