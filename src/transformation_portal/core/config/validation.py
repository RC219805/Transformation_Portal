...
def validate_config(config: Dict[str, Any]) -> List[str]:
        ...
        if isinstance(device, dict):
            ...
            if "memory_fraction" in device:
                ...
                if not (0.1 <= mem_frac <= 0.95):
                    errors.append(
                        f"device.memory_fraction must be between 0.1 and 0.95, got {mem_frac}"
                    )
        ...
        if isinstance(paths, dict):
            ...
            if isinstance(output, dict) and output.get("write_outputs", True):
                if not paths.get("output_dir"):
                    errors.append(
                        "paths.output_dir must be set when output.write_outputs is True"
                    )
    ...
    if "performance" in config:
        ...
        if isinstance(perf, dict):
            ...
            if tile_overlap >= tile_size:
                errors.append(
                    f"performance.tile_overlap ({tile_overlap}) must be less than tile_size ({tile_size})"
                )
            ...
            if batch_size > 64:
                errors.append(
                    f"performance.batch_size seems unreasonably large: {batch_size}"
                )
    ...
    if "output" in config:
        ...
        if isinstance(output, dict):
            ...
            if not (0.01 <= preview_scale <= 1.0):
                errors.append(
                    f"output.preview_scale must be between 0.01 and 1.0, got {preview_scale}"
                )

            # Check conflicting flags
            if output.get("skip_existing") and output.get("overwrite"):
                errors.append(
                    "output.skip_existing and output.overwrite cannot both be True"
                )
    ...
    if "validation" in config:
        ...
        if isinstance(val, dict):
            ...
            if max_size < 1.0:
                errors.append(
                    f"validation.max_input_size_mb must be >= 1.0, got {max_size}"
                )
