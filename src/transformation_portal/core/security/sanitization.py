...
def validate_input_file(path: Path, policy: Optional[SanitizationPolicy] = None) -> None:
    """
    ...
    if policy.allowed_extensions and ext not in policy.allowed_extensions:
        raise ValidationError(f"File type '{ext}' not allowed. Permitted: {policy.allowed_extensions}")
