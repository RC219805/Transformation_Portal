...
class DiffusionWorldModel(nn.Module):
    ...
    def __init__(
        self, latent_dim: int, cond_dim: int, T: int = 1000
    ) -> None:
        super().__init__()
    ...
    def forward(
        self, x0: torch.Tensor, t: torch.Tensor, cond: torch.Tensor
    ...
    @torch.no_grad()
    def sample(
        self, cond: torch.Tensor, steps: int = 50
    ) -> torch.Tensor:
        ...
        for i in reversed(range(steps)):
            t = torch.full(
                (B,), int(i * (T / steps)), device=cond.device, dtype=torch.long
            )
