...
class CoreMLExporter:
    ...
    def export_depth_model(
        ...
        if not TORCH_AVAILABLE or not COREML_AVAILABLE:
            ...
            print(
                f"CoreML export requires the following missing dependencies: {', '.join(missing)}"
            )
            return None
        ...
        try:
            ...
            mlmodel = ct.convert(
                ...
                inputs=[
                    ct.TensorType(
                        name="image", shape=example_input.shape, dtype=np.float32
                    )
                ],
                outputs=[ct.TensorType(name="depth")],
                compute_units=(
                    ct.ComputeUnit.ALL if optimize_for_ane else ct.ComputeUnit.CPU_ONLY
                ),
    ...
    def benchmark_model(
        ...
        try:
            ...
            dummy_input = {
                "image": np.random.randn(1, 3, input_size[0], input_size[1]).astype(
                    np.float32
                )
            }
...
class CoreMLDepthEstimator:
    ...
    def estimate(
        self, image: np.ndarray, target_size: Tuple[int, int] = (518, 518)
    ) -> np.ndarray:
        """
