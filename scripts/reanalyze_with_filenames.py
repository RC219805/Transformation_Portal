...
def get_filename_hint(filename):
    ...
    structure_patterns = ['kitchen', 'bathroom', 'bedroom', 'living', 'great', 'interior', 'entry', 'dining', 'office', 'courtyard']
...
def infer_expected(filename):
    ...
    for p in tex:
        if p in f: return 'texture_dominated'
    for p in struct:
        if p in f: return 'structure_dominated'
