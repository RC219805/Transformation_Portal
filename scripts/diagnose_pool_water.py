...
def main() -> int:
    ...
    masks_np = {k: v[0, 0].cpu().numpy().astype(np.float32) for k, v in masks_dict_torch.items()}
    ...
    if "water" in canonical_dict:
        ...
        water_candidates = [k for k in masks_np.keys() if normalize_material_name(k) == "water"]
    ...
    if "water" not in canonical_dict:
        ...
        water_related = [
            k for k in masks_np.keys() if any(w in k.lower() for w in ["water", "pool", "ocean", "lake", "pond", "sea"])
        ]
