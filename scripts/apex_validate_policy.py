...
def validate_schema_version(policy: Dict[str, Any], filename: str) -> List[str]:
...
def validate_performance_budgets(policy_dir: Path) -> List[str]:
    ...
    errors.extend(validate_schema_version(budgets, "performance_budgets.yaml"))
    ...
    for i, budget in enumerate(budgets['budgets']):
        ...
        if 'workflow_version' not in budget:
            ...
            errors.append(f"{prefix}: Invalid workflow_version '{budget['workflow_version']}' (must be {valid_workflow_versions})")
        ...
        if 'enforcement' not in budget:
            ...
            if 'mode' not in enforcement:
                ...
                errors.append(f"{prefix}.enforcement.mode: Invalid mode '{enforcement['mode']}' (must be {valid_enforcement_modes})")
...
def validate_enforcement_policy(policy_dir: Path) -> List[str]:
    ...
    # Validate schema version
    errors.extend(validate_schema_version(policy, "enforcement_policy.yaml"))
...
def validate_governance_rules(policy_dir: Path) -> List[str]:
    ...
    # Validate schema version
    errors.extend(validate_schema_version(rules, "governance_rules.yaml"))
        ...
        if 'expiry' in waivers:
            ...
            if 'default_days' in expiry and 'max_days' in expiry:
                if expiry['default_days'] > expiry['max_days']:
                    errors.append(f"governance_rules.yaml: waivers.expiry: default_days ({expiry['default_days']}) > max_days ({expiry['max_days']})")
    
    return errors


def validate_workload_suites(policy_dir: Path) -> List[str]:
    ...
    # Validate schema version
    errors.extend(validate_schema_version(suites, "workload_suites.yaml"))
...
def main() -> int:
    ...
    if args.check in ["all", "schema"]:
        ...
        errors.extend(validate_performance_budgets(args.policy_dir))
        errors.extend(validate_enforcement_policy(args.policy_dir))
        errors.extend(validate_governance_rules(args.policy_dir))
        errors.extend(validate_workload_suites(args.policy_dir))
