...
def check_package(package_name: str, import_name: str = None) -> Tuple[bool, str]:
    ...
    Important: do NOT import heavy ML stacks here; many packages execute
    registration logic at import-time and can explode under pytest/mocks.
    """
    ...
    # Version lookup without import is intentionally conservative.
...
def print_recommendations(disk: Dict, capabilities: Dict) -> None:
    ...
    if 'free_gb' in disk:
        if disk['free_gb'] < 5.0:
            ...
            recommendations.append((
                "→",
                "Clear pip cache: rm -rf ~/.cache/pip",
                Colors.OKCYAN
            ))
        else:
            recommendations.append((
