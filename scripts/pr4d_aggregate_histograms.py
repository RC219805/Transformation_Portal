...
def generate_markdown_report(data: AggregatedData, output_path: Path):
    ...
    ranked_materials = sorted(data.materials.values(), key=lambda m: m.recommendation_score(), reverse=True)
    ...
    with open(output_path, "w") as f:
        ...
        for stats in ranked_materials:
            ...
            f.write(
                f"| {stats.name} | {freq_str} | {coverage_str} | {boundary_str} | {edge_str} | "
        ...
        if top_material and top_material.name != "glass":
            ...
            if top_material.frequency == data.total_scenes:
                ...
                f.write(
                    f"- ✅ Appears in {top_material.frequency}/{data.total_scenes} scenes "
            ...
            if top_material.avg_boundary_pixels >= 250 and top_material.avg_edge_alignment >= 0.10:
                f.write(
                    f"- ✅ Strong edge signals (boundary_pixels: {top_material.avg_boundary_pixels:.0f}, "
                    f"edge_alignment: {top_material.avg_edge_alignment:.2f})\n"
                )
            else:
                f.write(
                    f"- ⚠️ Moderate edge signals (boundary_pixels: {top_material.avg_boundary_pixels:.0f}, "
                    f"edge_alignment: {top_material.avg_edge_alignment:.2f})\n"
                )
            ...
            f.write(
                f"- Edge Signals: {next_material.avg_boundary_pixels:.0f} boundary px, "
