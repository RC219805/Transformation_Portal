...
SCENE_CONFIGS = {
    ...
    "Pool": {
        ...
        "tone_map_params": {"key": 0.20, "sat": 0.88, "epsilon": 1e-6},
    },
    "PrimaryBathroom": {
...
def reinhard_local_tone_map(
    hdr_image: np.ndarray, key: float = 0.18, sat: float = 0.8, epsilon: float = 1e-6
    ...
    for c in range(3):
        ...
        if sat < 0.5:
            ...
            tone_mapped[:, :, c] = sat * tone_mapped[:, :, c] + (1 - sat) * tone_mapped_lum
...
def apply_depth_aware_clarity(image_array: np.ndarray, depth_map: np.ndarray, strength: float = 0.55) -> np.ndarray:
    """
...
def apply_atmospheric_haze(image_array: np.ndarray, depth_map: np.ndarray, density: float = 0.02) -> np.ndarray:
...
def apply_material_response(image_array: np.ndarray, depth_map: np.ndarray, strength: float = 0.75) -> np.ndarray:
    """
...
def apply_luxury_color_grade(image_array: np.ndarray, config: Dict[str, Any]) -> np.ndarray:
...
def main():
    ...
    if results:
        ...
        for result in results:
            ...
            print(
                f"{scene:<20} {w:>5}x{h:<7} {mp:>5.1f}  {master_size:>6.1f} MB  {web_size:>6.1f} MB  {proc_time / 60:>6.1f} min"
            )

        print("-" * 100)
        print(
            f"{'TOTAL':<20} {'':15} {'':7} {total_master_size:>6.1f} MB  {total_web_size:>6.1f} MB  {total_time / 60:>6.1f} min"
        )
        print()
