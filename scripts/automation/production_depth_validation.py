...
def process_single_image(
    ...
    try:
        ...
        metrics = validate_depth_quality(rgb, depth, dilation=3, save_heatmap=True, heatmap_path=heatmap_path)
        ...
        result.update({"error": error_msg, "traceback": error_trace})
...
def run_production_validation(
    ...
    refinement_config = {"edge_snap_strength": 0.2, "dilation": 5} if use_refinement else None
    ...
    for idx, rgb_path in enumerate(image_paths, 1):
        ...
        try:
            result = process_single_image(rgb_path, output_dir, config, refinement_config, force=force)
    ...
    if category_stats:
        ...
        for category, stats in category_stats.items():
            logger.info(
                f"{category.upper()}: {stats['quality_passed_strict']}/{stats['total']} strict pass, "
