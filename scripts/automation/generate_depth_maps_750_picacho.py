...
def create_multi_zone_visualization(depth: np.ndarray, zones: dict, original_size: Tuple[int, int]) -> Image.Image:
...
def process_image(
    ...
    if save_raw:
        ...
        depth_16bit = ((depth_array - depth_array.min()) / (depth_array.max() - depth_array.min()) * 65535).astype(np.uint16)
...
def main():
    ...
    for img_path in tiff_files:
        ...
        results.append({"input": img_path, "outputs": result})
