...
class UnifiedLuxuryPipeline:
    ...
    def process_image(self, input_path: Union[Path, str], output_path: Optional[Path] = None) -> PipelineResult:
        """
        ...
        # Initialize result
        result = PipelineResult(input_path=input_path, output_path=output_path, processing_time=0.0)
        ...
        if self.config.enable_material_response and self.material_responder:
            ...
            try:
                ...
                image = self.material_responder.enhance(image, surfaces=self.config.surface_types, depth_map=depth_map)
    ...
    def batch_process(
        self, input_paths: List[Path], progress_callback: Optional[callable] = None
