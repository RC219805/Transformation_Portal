...
def compare_results(new_results: Dict) -> Dict:
    ...
    for scene_key in PREVIOUS_RESULTS:
        ...
        if len(new_applied) > len(prev_applied):
            improvements.append(f"Fusion now applies to {len(new_applied)} classes (was {len(prev_applied)})")
        ...
        for cls in ["glass", "water", "foliage"]:
            ...
            if prev_iou_key in prev:
                ...
                new_iou = new.get("segmentation_v3", {}).get("per_class", {}).get(cls, {}).get("iou_base_vs_refined", 0.0)

                if new_iou > prev_iou + 0.05:  # meaningful improvement
                    improvements.append(f"{cls} IoU: {prev_iou:.3f} → {new_iou:.3f} (+{new_iou - prev_iou:.3f})")
                elif new_iou < prev_iou - 0.05:  # regression
                    regressions.append(f"{cls} IoU: {prev_iou:.3f} → {new_iou:.3f} ({new_iou - prev_iou:.3f})")
