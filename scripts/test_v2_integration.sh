...
check_prerequisites() {
    ...
    if [[ ! -f "${SCRIPT_DIR}/enhance_image.py" ]]; then
        print_error "V2 enhancement script not found: ${SCRIPT_DIR}/enhance_image.py"
        all_ok=false
    else
        print_success "V2 enhancement script found"
        log_verbose "Location: ${SCRIPT_DIR}/enhance_image.py"
    fi
