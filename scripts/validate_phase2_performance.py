...
class Phase2Validator:
    ...
    def run_benchmark(self, test_name: str, cli_args: List[str], config: Dict[str, Any]) -> BenchmarkResult:
        """
            ...
            if MEMORY_TRACKING_AVAILABLE:
                try:
                    ...
                    if platform.system() == "Darwin":
                        # macOS: ru_maxrss is in bytes, convert to MB
                        peak_from_rusage_mb = rusage.ru_maxrss / (1024 * 1024)
                    else:
                        # Linux and most other Unix-like systems: ru_maxrss is in KB, convert to MB
    ...
    def generate_report(self, output_file: Path):
        ...
        if baseline:
            ...
            for result in self.results:
                ...
                throughput_gain = (
                    (result.throughput_img_per_hour / baseline_throughput - 1) * 100 if baseline_throughput > 0 else 0
                )
...
def main():
    ...
    # Initialize validator
    validator = Phase2Validator(test_dir=Path(args.test_dir), output_base=Path(args.output_dir))
