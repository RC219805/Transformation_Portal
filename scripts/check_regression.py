...
def check_regression(baseline_path: Path, current_path: Path, mode: str) -> bool:
    ...
    pool_edge_drop = baseline_summary["pool_avg_edge_alignment"] - current_summary["pool_avg_edge_alignment"]
    ...
    ocean_edge_drop = baseline_summary["ocean_avg_edge_alignment"] - current_summary["ocean_avg_edge_alignment"]
    ...
    if ft_increase > 0.15:  # Absolute: +0.15 (e.g., 0.05 → 0.20)
        warnings.append(f"False trigger rate increased by {ft_increase:.1%} ({baseline_ft:.1%} → {current_ft:.1%})")
