...
def rank_materials(material_stats: Dict[str, Dict]) -> List[tuple]:
    ...
    for material, stats in material_stats.items():
        ...
        # Weighted priority
        priority = frequency_score * 0.3 + coverage_score * 0.25 + confidence_score * 0.15 + signal_score * 0.30
...
def main():
    ...
    stats_output = {
        "num_scenes": len(reports),
        "materials": {material: {**stats, "priority": priority} for material, priority, stats in ranked},
    ...
    if ranked:
        ...
        for rank, (material, priority, stats) in enumerate(ranked[:3], 1):
            no_impl = stats["pixel_ops_reasons"].get("no_implementation", 0)
            print(
                f"  {rank}. {material:12s}  priority={priority:.3f}  "
