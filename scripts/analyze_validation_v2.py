...
def infer_ground_truth(filename: str) -> str:
    ...
    # Texture patterns
    texture_patterns = [
        'pool', 'ocean', 'water', 'glass', 'aerial',
        'foliage', 'trees', 'shores', 'beach', 'sea'
    ]
    
    # Structure patterns
    structure_patterns = [
        'kitchen', 'bathroom', 'bedroom', 'living', 'great',
        'interior', 'entry', 'dining', 'office', 'courtyard',
        'room', 'hall', 'lobby'
    ]
...
def compute_pass_flags(metrics: Dict) -> Tuple[bool, bool]:
    ...
    passed_lenient = (
        edge_f1 >= 0.30 and
        edge_overlap >= 0.40 and
        edge_count_ratio <= 3.0 and
    ...
    passed_strict = (
        edge_f1 >= 0.45 and
        edge_overlap >= 0.50 and
        edge_count_ratio <= 2.0 and
        halo_score >= 0.7 and
...
def plot_feature_separability(metrics_list: List[Dict], output_path: Path):
    ...
    for idx, (key, label) in enumerate(features):
        ...
        ax.scatter(
            ...
            c='blue', alpha=0.6, s=100, label='Structure'
        )
        ax.scatter(
            texture_data[key],
            [1] * len(texture_data[key]),
            c='red', alpha=0.6, s=100, label='Texture'
        )
