...
def visualize_comparison(depth1: np.ndarray, depth2: np.ndarray, output_dir: Path, prefix: str = "comparison") -> None:
    """
...
def main() -> int:
    ...
    if depth_master.shape != depth_upscaled.shape:
        ...
        depth_upscaled_pil = depth_upscaled_pil.resize(
            (depth_master.shape[1], depth_master.shape[0]), PILImage.Resampling.BICUBIC
        )
