...
def apply_pbr_overlays(
    ...
    *, albedo: Optional[Union[str, Path]] = None, variant_map: Optional[Union[str, Path]] = None, variant_mix: float = 0.0,
    ...
    if albedo is not None:
        alb = _open_rgb(albedo)
        alb = _resize(alb, (w, h))
        alb_np = _srgb_to_linear(_as_f32_rgb(alb))
        if variant_map:
            v2 = _open_rgb(variant_map)
            v2 = _resize(v2, (w, h))
    ...
    if roughness is not None:
        r_im = _open_rgb(roughness)
        r_im = _resize(r_im, (w, h))
        r_np = _as_f32_rgb(r_im)
    ...
    if metallic_map is not None:
        m_im = _open_rgb(metallic_map)
        m_im = _resize(m_im, (w, h))
        m_np = _as_f32_rgb(m_im).mean(axis=-1)
    ...
    if env_map is not None:
        try:
            env_im = _open_rgb(env_map)
            env_np = _as_f32_rgb(env_im)
            env = env_np
        except Exception:
            env = None
