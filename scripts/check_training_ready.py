...
def check_python_version():
    ...
    if passed:
        ...
        print(f"   Install Python 3.10+ from https://www.python.org/")
...
def check_pytorch():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            print(f"   Training will be FAST (~3-4 hours)")
            return True
        
        # Check MPS (Apple Silicon)
        if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
            print_check(True, f"MPS available (Apple Silicon)")
            print(f"   Training will be FAST (~2.5-3.5 hours)")
            return True
        
        # CPU only
        print_check(True, f"CPU only (no GPU detected)")
        print(f"   ⚠️  Training will be SLOW (~12-18 hours)")
        print(f"   Consider using synthetic data (--quickstart) for faster results")
        return True
    ...
    try:
        ...
        try:
            ...
            if passed:
                ...
                if avail_gb >= 16:
                    print(f"   💪 Plenty of memory - can use batch size 4-8")
                elif avail_gb >= 12:
                    print(f"   👍 Good memory - can use batch size 4")
                else:
                    print(f"   ⚠️  Minimum memory - use batch size 2")
            else:
            ...
            if Path('/proc/meminfo').exists():
                ...
                print(f"   Ensure you have at least 8 GB RAM available")
                return True
...
def check_training_data():
    ...
    if not picacho_available:
        ...
        print(f"   No problem - can use synthetic data instead")
        print(f"   Run: ./scripts/quickstart_training.sh")
