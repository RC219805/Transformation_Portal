...
def stratified_analysis(
    ...
    for stratum_val in baseline[stratify_by].dropna().unique():
        ...
        strata.append({
            ...
            "lenient_rate_delta": (
                treatment_stratum["lenient_pass"].mean() - baseline_stratum["lenient_pass"].mean()
            ),
            "baseline_edge_f1_mean": baseline_stratum["edge_f1"].mean(),
            "treatment_edge_f1_mean": treatment_stratum["edge_f1"].mean(),
            "edge_f1_delta": (
                treatment_stratum["edge_f1"].mean() - baseline_stratum["edge_f1"].mean()
            ),
        })
...
def main():
    parser = argparse.ArgumentParser(
        description="Statistical comparison of multi-model validation results"
    )
    parser.add_argument(
        "--comparison-dir",
        type=Path,
    ...
    if not args.treatment_models:
        ...
        args.treatment_models = [
            d.name.replace("model_", "")
            for d in model_dirs
            if d.name != f"model_{args.baseline_model}"
        ]
    ...
    for treatment_key in args.treatment_models:
        ...
        for metric in metrics_to_test:
            ...
            if "error" not in result:
                ...
                print(f"  [{sig}] {metric}: Δ={result['mean_diff']:.4f}, p={result['p_value']:.4f}, d={result['cohens_d']:.2f}")
