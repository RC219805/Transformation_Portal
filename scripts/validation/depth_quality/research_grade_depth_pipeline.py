...
def apply_guided_filter(depth: np.ndarray, guide: np.ndarray, radius: int = 8, eps: float = 0.01) -> np.ndarray:
    ...
    try:
        ...
        filtered = cv2.ximgproc.guidedFilter(guide=guide, src=depth.astype(np.float32), radius=radius, eps=eps)
...
def run_research_grade_pipeline(input_path: Path, output_dir: Path):
    ...
    depth_16bit = normalize_depth_advanced(depth=filtered_depth, percentile_low=0.5, percentile_high=99.5, gamma=1.0)
