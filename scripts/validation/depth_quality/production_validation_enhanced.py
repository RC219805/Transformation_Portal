...
def compute_edge_metrics_detailed(rgb: np.ndarray, depth: np.ndarray, tolerance_px: int = 2) -> Dict[str, float]:
    """
...
def compute_overshoot_penalty(rgb: np.ndarray, depth: np.ndarray) -> Tuple[float, np.ndarray]:
    ...
    logger.debug(
        f"Overshoot: edge_p95={edge_p95:.3f}, interior_p95={interior_p95:.3f}, ratio={ratio:.2f}, penalty={penalty:.3f}"
    )

    return float(penalty), heatmap


def create_readable_overlay(rgb: np.ndarray, depth: np.ndarray, output_path: Path) -> None:
    """
...
def validate_seams_enhanced(
    depth: np.ndarray, tile_size: int = 1024, overlap: int = 128, band: int = 4
...
def process_single_image(rgb_path: Path, output_dir: Path, config: DepthConfig, force: bool = False) -> ValidationResult:
    """
    ...
    result = ValidationResult(image_name=image_name, rgb_path=str(rgb_path), execution_success=False)
    ...
    try:
        ...
        # Seam validation
        seam_passed, seam_ratio, seam_heatmap = validate_seams_enhanced(depth, config.tile_size, config.overlap, band=4)
        ...
        result.passed_lenient = (
            result.edge_f1 >= 0.30 and result.chamfer_distance < 15.0 and result.edge_count_ratio <= 2.0 and seam_passed
        )

        result.passed_strict = (
            result.edge_f1 >= 0.60
            and result.edge_overlap >= 0.70
            and result.chamfer_distance < 5.0
            and result.edge_count_ratio <= 1.5
            and result.overshoot_penalty < 0.3
            and seam_passed
        )
