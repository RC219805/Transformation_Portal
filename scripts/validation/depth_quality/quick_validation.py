...
def main():
    ...
    logger.info(
        f"✓ Quality: {metrics_tiled.quality_score():.3f} (Δ{metrics_tiled.quality_score() - metrics_baseline.quality_score():+.3f})"
    )
    ...
    logger.info(
        f"✓ Sharpness: {metrics_refined.edge_sharpness_p95:.1f} (Δ{metrics_refined.edge_sharpness_p95 - metrics_tiled.edge_sharpness_p95:+.1f})"
    )
