...
def main():
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        logger.info(
            f"  HiFi edge alignment:     {hifi_metrics.edge_alignment:.3f} ({comparison['edge_alignment']['delta']:+.3f})"
        )
    ...
    logger.info(
        f"Average edge overlap:   {avg_overlap_baseline:.3f} → {avg_overlap_hifi:.3f} ({avg_overlap_hifi - avg_overlap_baseline:+.3f})"
    )
