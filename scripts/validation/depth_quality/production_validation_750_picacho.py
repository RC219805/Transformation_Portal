...
class EnhancedMetrics:
    ...
    def quality_gate_passed(self, strict: bool = False) -> bool:
        ...
        if strict:
            return (
                self.edge_f1 >= 0.45
                and self.edge_precision >= 0.40
                and self.edge_recall >= 0.40
                and self.edge_count_ratio <= 2.0
                and self.seam_passed
                and self.halo_score >= 0.70
                and self.overshoot_penalty <= 0.30
                and self.detail_benefit >= 1.0  # Tiled should add real detail
            )
        else:
            return (
                self.edge_f1 >= 0.30
                and self.edge_precision >= 0.25
                and self.edge_recall >= 0.25
                and self.edge_count_ratio <= 3.0
                and self.seam_passed
                and self.overshoot_penalty <= 0.50
            )


def compute_edge_precision_recall(depth_edges: np.ndarray, rgb_edges: np.ndarray, tolerance: int = 2) -> Tuple[float, float]:
    """
...
def compute_halo_overshoot_metrics(depth: np.ndarray, rgb_edges: np.ndarray) -> Tuple[float, float, float]:
    """
...
def compute_detail_benefit_metric(
    depth_tiled: np.ndarray, depth_baseline: np.ndarray, window_size: int = 16
) -> Tuple[float, float]:
    """
...
def validate_seams(depth: np.ndarray, tile_size: int, overlap: int, band: int = 2) -> Tuple[bool, float]:
...
def process_single_image(
    ...
    try:
        ...
        # 4. Seam validation
        seam_passed, seam_ratio = validate_seams(depth_final, preset.tile_size, preset.overlap)

        # 5. Halo/overshoot metrics
        halo_score, overshoot_penalty, max_slope = compute_halo_overshoot_metrics(depth_final, rgb_edges)

        # 6. Detail benefit
        detail_benefit, hf_energy_ratio = compute_detail_benefit_metric(depth_tiled, depth_baseline)
...
def run_production_validation(
    ...
    logger.info(
        f"  Chamfer Distance: {aggregate['chamfer_distance']['mean']:.2f} ± {aggregate['chamfer_distance']['std']:.2f} px"
    )
    logger.info(
        f"  Edge Count Ratio: {aggregate['edge_count_ratio']['mean']:.2f} ± {aggregate['edge_count_ratio']['std']:.2f}×"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Production validation on full 750_Picacho dataset")
    parser.add_argument(
        "--input-dir",
        type=Path,
