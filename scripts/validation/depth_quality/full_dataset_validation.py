...
def validate_single_image(
    ...
    if config.tile_size > 0:
        ...
        seam_detected, seam_energy = validate_seams(depth_norm, config.tile_size, config.overlap, band=2)
...
def main():
    ...
    for img_path in image_paths:
        try:
            metrics = validate_single_image(img_path, estimator, val_config, args.output_dir)
            results.append(metrics)

            status = "✓ PASS" if metrics.passed else "✗ FAIL"
            logger.info(
                f"{status} {img_path.name}: F1={metrics.edge_f1:.3f}, "
