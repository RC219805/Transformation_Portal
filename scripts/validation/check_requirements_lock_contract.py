...
FORBIDDEN_GENERIC_PROVENANCE_OPTIONS = ("--no-index",)
...
SUPPORTED_TRANSFORMERS_MIN = "5.10.1"
SUPPORTED_TRANSFORMERS_INPUT_RANGE = "transformers>=5.10.1,<5.11"
...
def validate_lockfile_headers(
    ...
    *,
    ...
    for lock_name in target_locks:
        ...
        header_lines = lock_path.read_text(encoding="utf-8").splitlines()[:8]
        header_match = None
        ...
        if lock_name in CORE_LOCK_FILES:
            command_tokens = {
                token for line in header_lines if line.startswith("#    ") for token in line.removeprefix("#    ").split()
            }
            for option in FORBIDDEN_GENERIC_PROVENANCE_OPTIONS:
                if option in command_tokens:
                    errors.append(
                        f"{lock_path} records forbidden compiler provenance option {option!r}; "
