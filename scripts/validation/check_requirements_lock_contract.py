...
PLATFORM_LOCK_FORBIDDEN_MARKERS = {
    "ml-core-darwin.txt": ('platform_system == "Linux"',),
    "ml-core-linux.txt": ('platform_system == "Darwin"',),
}
...
def validate_platform_lock_markers() -> list[str]:
    ...
    for lock_name, forbidden_markers in PLATFORM_LOCK_FORBIDDEN_MARKERS.items():
        ml_lock_path = REQUIREMENTS_DIR / lock_name
        if not ml_lock_path.is_file():
            continue
        ...
        for line_number, raw_line in enumerate(ml_lock_path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            for marker in forbidden_markers:
                if marker in line:
                    errors.append(
                        f"{ml_lock_path}:{line_number} contains forbidden platform marker {marker!r} "
