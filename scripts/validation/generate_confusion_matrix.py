...
def generate_confusion_matrix(metrics: Dict[str, dict], expected: Dict[str, str]) -> Tuple[Dict, int, int]:
    """
...
def print_confusion_matrix(confusion: Dict, correct: int, total: int, metrics: Dict[str, dict]):
    ...
    all_labels = sorted(set(list(confusion.keys()) + [pred for preds in confusion.values() for pred in preds.keys()]))
    ...
    for image_name, data in sorted(metrics.items()):
        ...
        if predicted != actual and actual != "unknown":
            ...
            print(
                f"     Factors: ratio={factors.get('ratio', 0):.2f}, "
...
def print_quality_summary(metrics: Dict[str, dict]):
    ...
    by_scene = defaultdict(lambda: {"total": 0, "lenient_pass": 0, "strict_pass": 0, "edge_f1": []})
    ...
    for scene_type, stats in sorted(by_scene.items()):
        ...
        print(
            f"  Lenient pass: {stats['lenient_pass']}/{stats['total']} ({100 * stats['lenient_pass'] / stats['total']:.1f}%)"
        )
        print(f"  Strict pass:  {stats['strict_pass']}/{stats['total']} ({100 * stats['strict_pass'] / stats['total']:.1f}%)")

        if stats["edge_f1"]:
            import numpy as np

            print(
                f"  Edge F1: mean={np.mean(stats['edge_f1']):.3f}, "
