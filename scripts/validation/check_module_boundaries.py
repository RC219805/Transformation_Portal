...
class ModuleBoundaryChecker:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    violation = self._check_import(filepath, node.lineno, importing_layer, alias.name)
                    if violation:
                        violations.append(violation)

            elif isinstance(node, ast.ImportFrom):
                if node.module:
                    violation = self._check_import(filepath, node.lineno, importing_layer, node.module)
                    if violation:
                        violations.append(violation)
    ...
    def _check_import(
        self, filepath: Path, lineno: int, importing_layer: str, imported_module: str
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Check module boundary violations in transformation_portal")
    parser.add_argument(
        "--src-dir",
        type=Path,
