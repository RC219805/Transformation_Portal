...
def main():
    ...
    for depth_path in sorted(depth_files):
        ...
        try:
            ...
            logger.info(
                f"  ✅ edge_f1={metrics['edge_f1']:.3f}, chamfer={metrics['chamfer_distance']:.1f}px, quality={metrics['quality_score']:.3f}"
            )
            logger.info(
                f"      lenient={'PASS' if metrics['passed_lenient'] else 'FAIL'}, strict={'PASS' if metrics['passed_strict'] else 'FAIL'}"
            )
