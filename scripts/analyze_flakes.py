...
def format_text(stats: dict[str, Any]) -> str:
    ...
    lines.append("=" * 60)
    lines.append(f"Total tests tracked: {stats['total_tests']}")
    lines.append(
        f"  Stable:        {stats['stable']} ({stats['stable']/stats['total_tests']*100 if stats['total_tests'] else 0:.1f}%)"
    )
    lines.append(
        f"  Monitored:     {stats['monitored']} ({stats['monitored']/stats['total_tests']*100 if stats['total_tests'] else 0:.1f}%)"
    )
    lines.append(
        f"  Quarantined:   {stats['quarantined']} ({stats['quarantined']/stats['total_tests']*100 if stats['total_tests'] else 0:.1f}%)"
    )
    lines.append("")
    ...
    if stats["flaky_tests"]:
        ...
        for i, test in enumerate(stats["flaky_tests"], 1):
            ...
            lines.append(f"   Flake rate: {test['flake_rate']*100:.2f}% ({test['flake_count']}/{test['total_runs']} runs)")
...
def format_markdown(stats: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append(
        f"**Repository-wide flake rate:** {stats['repo_flake_rate']*100:.2f}% ({stats['total_flakes']} flakes / {stats['total_runs']} runs)"
    )
    lines.append("")
    lines.append(
        f"**Thresholds:** Flake = {stats['flake_threshold']*100:.1f}%, Quarantine = {stats['quarantine_threshold']*100:.1f}%"
    )
    lines.append("")
    ...
    if stats["flaky_tests"]:
        ...
        for test in stats["flaky_tests"]:
            ...
            lines.append(
                f"| `{test_name}` | {status_icon} | {test['flake_rate']*100:.2f}% | {test['flake_count']}/{test['total_runs']} |"
            )
        lines.append("")
    else:
        lines.append("✅ **No flaky tests detected!**")
