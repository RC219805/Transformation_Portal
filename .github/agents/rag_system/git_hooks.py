...
class ChangeDetector:
    ...
    def get_last_commit_changes(self) -> List[FileChange]:
        ...
        code, stdout, _ = self._run_git("diff-tree", "--no-commit-id", "--name-status", "-r", "HEAD")
...
class IncrementalIndexer:
    ...
    def _load_state(self) -> None:
        ...
        if state_file.exists():
            try:
                ...
                logger.warning(
                    f"Failed to load incremental indexer state from {state_file}: {e}. Proceeding without previous state."
                )
    ...
    def update_index(
        ...
        logger.info(f"Processing {len(changes)} changes: +{len(added)} ~{len(modified)} -{len(deleted)} R{len(renamed)}")
...
class GitHookManager:
    ...
    def update_now(self) -> Dict[str, Any]:
        ...
        changes = self.detector.get_changes_since_commit(self.indexer._last_indexed_commit)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Transformation Portal RAG System - Git Hook Integration")
