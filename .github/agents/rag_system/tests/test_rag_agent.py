...
try:
    ...
    spec = importlib.util.spec_from_file_location("rag_agent", rag_system_path / "rag_agent.py")
...
class TestRAGAgent:
    ...
    def test_query_with_context(self, temp_repo):
        ...
        context = QueryContext(user_intent=UserIntent.IMPLEMENTATION, priority="high")

        response = agent.query("How to add atmospheric effects?", context=context)
    ...
    def test_prepare_context_for_agent(self, temp_repo):
        ...
        context = agent.prepare_context_for_agent("transformation-portal-specialist", "Add atmospheric haze effect")
    ...
    def test_query_decomposition(self, temp_repo):
        ...
        sub_queries = agent._decompose_query("add atmospheric effect and update documentation", context)
    ...
    def test_adaptive_strategy_with_feedback(self, temp_repo):
        ...
        context = QueryContext(user_intent=UserIntent.IMPLEMENTATION)
        agent.query("add new feature", context=context, strategy=RetrievalStrategy.MULTI_SOURCE)
    ...
    def test_citation_generation_in_response(self, temp_repo):
        ...
        response = agent.query("depth processing", include_citations=True, max_citations=3)
