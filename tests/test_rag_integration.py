...
class TestRAGIntegration:
    ...
    def test_pipeline_finds_documentation(self, rag_pipeline):
        ...
        # Filter to only docs
        results = rag_pipeline["retriever"].retrieve(query, top_k=5, chunk_type_filter=["doc"])
    ...
    def test_pipeline_filters_by_test_files(self, rag_pipeline):
        ...
        results = rag_pipeline["retriever"].retrieve(query, top_k=5, chunk_type_filter=["test"])
    ...
    def test_pipeline_handles_specific_file_search(self, rag_pipeline):
        ...
        results = rag_pipeline["retriever"].retrieve(query, top_k=5, file_path_filter=r"material_response")
    ...
    def test_feature_template_with_rag_context(self, rag_pipeline):
        ...
        # Format citations
        citation_text = rag_pipeline["citation_gen"].format_citations(citations, format_type="markdown")

        # Generate template with context
        template = PromptTemplates.feature_implementation("Add new sunset LUT preset", context=citation_text)
    ...
    def test_bug_triage_template_with_examples(self, rag_pipeline):
        ...
        # Generate template
        template = PromptTemplates.bug_triage(error_log, environment="Python 3.10")

        # Add examples
        template_with_examples = PromptTemplates.add_few_shot_examples(template, examples)
    ...
    def test_pipeline_performance(self, rag_pipeline):
        ...
        assert rerank_time < rerank_threshold, f"Reranking took {rerank_time:.3f}s (expected < {rerank_threshold}s)"
        assert citation_time < citation_threshold, f"Citation took {citation_time:.3f}s (expected < {citation_threshold}s)"

        total_time = retrieval_time + rerank_time + citation_time
        assert total_time < total_threshold, f"Total pipeline took {total_time:.3f}s (expected < {total_threshold}s)"
