...
class TestSceneClassification:
    ...
    def test_confident_classification(self):
        ...
        scene = SceneClassification(scene_type=SceneType.INTERIOR, scene_subtype="kitchen", confidence=0.85)
        assert scene.is_confident is True

    def test_low_confidence_classification(self):
        """Test low confidence classification (< 0.5)."""
        scene = SceneClassification(scene_type=SceneType.INTERIOR, scene_subtype="kitchen", confidence=0.35)
        assert scene.is_confident is False

    def test_threshold_confidence(self):
        """Test confidence exactly at threshold."""
        scene = SceneClassification(scene_type=SceneType.EXTERIOR, scene_subtype="pool", confidence=0.5)
        assert scene.is_confident is True
...
class TestPresetSelector:
    ...
    def test_classify_scene_interior_kitchen(self):
        """Test classification of interior kitchen scene."""
        # Patch before PresetSelector instantiation
        with patch.object(PresetSelector, "__init__", lambda self, *args, **kwargs: None):
            selector = PresetSelector()

            # Manually set up the mock clip
            mock_clip = Mock()
            selector.clip = mock_clip
            selector.confidence_threshold = 0.5

            # Mock CLIP responses
            mock_clip.classify_image.side_effect = [
    ...
    def test_classify_scene_exterior_pool(self):
    ...
    def test_select_preset_high_confidence(self):
    ...
    def test_select_preset_low_confidence_fallback(self):
    ...
    def test_select_preset_from_path(self, tmp_path):
