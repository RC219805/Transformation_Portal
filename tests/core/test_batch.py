...
def test_job_item_creation():
    ...
    item = JobItem(input_path="input.jpg", output_path="output.jpg")
...
def test_batch_processor_creation(tmp_path):
    ...
    processor = BatchProcessor(processor_fn=dummy_processor, checkpoint_dir=tmp_path, max_retries=3)
...
def test_batch_processor_process_batch(tmp_path):
    ...
    batch_processor = BatchProcessor(processor_fn=processor, checkpoint_dir=tmp_path / "checkpoints")
...
def test_batch_processor_resume(tmp_path):
    ...
    batch_processor2 = BatchProcessor(processor_fn=working_processor, checkpoint_dir=checkpoint_dir)

    checkpoint_path = checkpoint_dir / "test_job.json"
    resumed_job = batch_processor2.process_batch([], output_dir, resume_from=checkpoint_path)
