...
def test_water_candidate_report_schema_present(rgb_dummy, segmentation_result_no_water):
...
def test_water_audit_in_class_presence(rgb_dummy, segmentation_result_no_water):
    """Class presence audit includes water metrics.
...
def test_water_audit_with_segformer_water(rgb_dummy, segmentation_result_with_water):
...
def test_no_behavior_change_when_disabled(rgb_dummy, segmentation_result_no_water):
    ...
    config_disabled = MaterialsV3Config(enabled=True, water_detection_enabled=False)
    engine_disabled = MaterialsV3Engine(config_disabled)
    result_disabled = engine_disabled.process(rgb_dummy, segmentation_result_no_water)
    ...
    result_baseline = engine_baseline.process(rgb_dummy, segmentation_result_no_water)
    ...
    for material_name in materials_disabled.keys():
        ...
        if isinstance(mask_disabled, np.ndarray) and isinstance(mask_baseline, np.ndarray):
...
def test_multiple_process_calls_consistent(rgb_dummy, segmentation_result_no_water):
...
def test_sample_prompts_from_mask():
    ...
    prompts = engine._sample_prompts_from_mask(mask, confidence_threshold=0.7, num_samples=5)
...
def test_roi_operations():
    ...
    np.testing.assert_array_equal(mask_full[y0:y1, x0:x1], mask[y0:y1, x0:x1])
