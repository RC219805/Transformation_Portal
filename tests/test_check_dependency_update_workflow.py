...
TOOL_PATH = (
    PROJECT_ROOT / "scripts" / "validation" / "check_dependency_update_workflow.py"
)
SPEC = importlib.util.spec_from_file_location(
    "check_dependency_update_workflow", TOOL_PATH
)
...
def valid_workflow_text() -> str:
    ...
    required_pr_refs = "\n".join(
        f"          {ref}" for ref in workflow_contract.REQUIRED_PR_BODY_REFERENCES
    )
    return f"""
...
def test_missing_required_audit_target_is_reported() -> None:
    ...
    assert (
        "dependency-update workflow must audit governed lockfile target "
...
def test_stale_noncontract_ml_reference_is_reported() -> None:
    ...
    errors = workflow_contract.validate_dependency_update_workflow(broken)
    assert (
        "dependency-update workflow still references non-contract ML lockfile "
        "'requirements/ml.txt'"
    ) in errors
...
def test_missing_required_pr_body_reference_is_reported() -> None:
    ...
    errors = workflow_contract.validate_dependency_update_workflow(broken)
    assert (
        "dependency-update PR body must reference checked-in contract file "
        "'requirements/security.txt'"
    ) in errors
