...
def test_stability_deterministic_with_seed():
    ...
    assert abs(stability1 - stability2) < 1e-6, f"Stability scores differ with same seed: {stability1} vs {stability2}"
...
def test_stability_different_with_different_seed():
    ...
    config = MaterialsV3Config(water_detection_enabled=True, water_edge_refinement_enabled=False)
...
def test_full_validation_deterministic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert abs(stability1 - stability2) < 1e-6, f"Full validation stability differs: {stability1} vs {stability2}"
