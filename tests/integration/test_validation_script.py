...
def test_validation_script_calls_v2_classifier(test_image_dir, tmp_path):
    ...
    for mf in metrics_files:
        ...
        assert metrics["scene_type"] is not None, f"scene_type is null in {mf.name} - V2 classifier not called!"

        assert metrics["edge_f1"] is not None, f"edge_f1 is null in {mf.name}"

        assert isinstance(metrics["edge_f1"], (int, float)), f"edge_f1 must be numeric, got {type(metrics['edge_f1'])}"

        assert metrics["lenient_pass"] is not None, f"lenient_pass is null in {mf.name}"

        assert isinstance(metrics["lenient_pass"], bool), f"lenient_pass must be bool, got {type(metrics['lenient_pass'])}"

        assert metrics["classification_factors"] is not None, f"classification_factors is null in {mf.name}"

        assert "ratio" in metrics["classification_factors"], f"classification_factors missing 'ratio' in {mf.name}"
