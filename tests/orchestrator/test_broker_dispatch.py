...
def test_broker_construction_failure_cleans_runtime_when_repo_delete_fails(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Rollback cleanup must not short-circuit before clearing local side effects."""
    ...
    class DeleteFailingRepository:
        ...
        async def delete(self, job_id: str) -> None:
            raise RuntimeError(f"simulated delete failure for {job_id}")

        def __getattr__(self, name: str) -> Any:
            return getattr(self._repository, name)

    with TestClient(orchestrator_app.app, headers={"x-api-key": "contract-secret"}) as client:
        orchestrator_app.app.state.job_repository = DeleteFailingRepository(orchestrator_app.app.state.job_repository)
        response = client.post("/v1/jobs", json=payload)

    assert response.status_code == 503, response.text
    assert response.json()["error"]["code"] == "JOB_REPOSITORY_UNAVAILABLE"
    assert created_ids
    assert created_ids[0] not in orchestrator_app.JOBS
    assert created_ids[0] not in orchestrator_app.EVENT_SUBSCRIBERS
    assert not output_dir.exists(), "rollback must still reclaim the output_dir it created before delete failed"
