...
def sample_ledger(tmp_path: Path) -> Path:
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='apex_runs'"
        )
        assert cursor.fetchone() is not None, "apex_runs table should exist"
        
        cursor = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='view' AND name='apex_trends'"
        )
...
def test_optimized_indexes_exist(sample_ledger: Path) -> None:
    ...
    with sqlite3.connect(sample_ledger) as conn:
        cursor = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='apex_runs'"
        )
...
def test_query_performance_with_indexes(sample_ledger: Path) -> None:
    ...
    with sqlite3.connect(sample_ledger) as conn:
        ...
        cursor = conn.execute(
            "SELECT * FROM apex_runs ORDER BY timestamp DESC LIMIT 100"
        )
        cursor.fetchall()
...
def test_ledger_migration_v2_to_v3(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        # Check indexes exist
        cursor = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_apex_runs_timestamp'"
        )
