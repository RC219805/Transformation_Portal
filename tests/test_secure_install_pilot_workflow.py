...
def test_secure_install_pilot_workflow_runs_makefile_targets_and_uploads_artifacts() -> None:
    ...
    assert 'python -m pip install "pip-tools==7.6.1" "click==8.4.2"' in install_step["run"]
...
def test_secure_install_pilot_readme_mentions_local_toolchain_requirement() -> None:
    ...
    assert 'python -m pip install "pip-tools==7.6.1" "click==8.4.2"' in readme
...
def test_requirements_readme_records_current_curated_web_runtime_baseline() -> None:
    ...
    assert "| FastAPI | `requirements/base.in` | `0.141.1` |" in readme
    assert "| Starlette | `requirements/base.in` + `pyproject.toml` bound | `1.6.0` |" in readme
    assert "| Uvicorn | `requirements/base.in` + `pyproject.toml` bound | `0.52.1` |" in readme
    assert "FastAPI `0.141.1` update in PR #2042" in readme
    assert "Starlette `1.6.0` promotion on 2026-08-28" in readme
