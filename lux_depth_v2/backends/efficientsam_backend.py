...
class EfficientSAMBackend:
    ...
    def _prepare_onnx_inputs(
        ...
        Stage 5A: Implements actual efficientsam_s.onnx I/O contract.
        Model expects:
        - batched_images: [batch, 3, height, width], float32
        - batched_point_coords: [1, 1, num_points, 2], float32 (pixel coords)
        - batched_point_labels: [1, 1, num_points], float32
    ...
    def _postprocess_outputs(
        ...
        Stage 5A: Handles efficientsam_s.onnx output format.

        Expected outputs (from model introspection):
        - output_masks: [1, 1, num_masks, height, width], float32 (logits)
        - iou_predictions: [1, 1, num_masks], float32
        ...
        if mask_logits.ndim == 5:
            ...
            if iou_preds is not None and iou_preds.size > 0:
                ...
                log.debug(f"Selecting mask {best_idx} with IoU={iou_flat[best_idx]:.4f}")
                mask_logits_2d = logits_3d[best_idx]
            else:
                # Fallback: use first mask
                mask_logits_2d = logits_3d[0]
        elif mask_logits.ndim == 4:  # Fallback: (1, 1, H, W)
            mask_logits_2d = mask_logits[0, 0]
        elif mask_logits.ndim == 3:  # (1, H, W)
            mask_logits_2d = mask_logits[0]
        elif mask_logits.ndim == 2:  # (H, W)
            mask_logits_2d = mask_logits
        else:
            raise ValueError(f"Unexpected output shape: {mask_logits.shape}")

        # Apply sigmoid to convert logits to probabilities
        # EfficientSAM outputs raw logits, need sigmoid activation
        mask = 1.0 / (1.0 + np.exp(-mask_logits_2d.astype(np.float32)))
