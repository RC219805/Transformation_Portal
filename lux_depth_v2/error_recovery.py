...
class ErrorRecovery:
    ...
    def __init__(self, strategy: Optional[RetryStrategy] = None, logger=None):
        ...
        self.logger.info(
            f"ErrorRecovery initialized | max_retries={self.strategy.max_retries} backoff_base={self.strategy.backoff_base}"
        )
    ...
    def should_retry(self, error: Exception, task_id: str, attempt: int) -> Tuple[bool, str]:
    ...
    def get_fallback_config(self, original_config: Dict[str, Any], error: Exception, attempt: int) -> Dict[str, Any]:
    ...
    def execute_with_retry(self, func: Callable, task_id: str, *args, **kwargs) -> Tuple[Any, bool, Optional[str]]:
        ...
        for attempt in range(self.strategy.max_retries + 1):
            try:
                ...
                # Success
                if attempt > 0:
                    self.logger.info(f"Retry successful | task_id={task_id} attempt={attempt}")
                ...
                if not should_retry:
                    self.logger.error(f"Permanent failure | task_id={task_id} attempt={attempt} reason={reason} error={e}")
...
def with_retry(error_recovery: ErrorRecovery, task_id: str):
    ...
    def decorator(func: Callable):
        def wrapper(*args, **kwargs):
            result, success, error = error_recovery.execute_with_retry(func, task_id, *args, **kwargs)
            if not success:
...
class MaterialsV2FallbackStrategy:
    ...
    def get_fallback_config(self, error: Exception, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "memory" in error_str or "oom" in error_str:
            ...
            if new_size >= 512:  # Minimum viable size
                ...
                self.logger.warning(f"Materials v2 memory fallback: {current_size}px → {new_size}px")
                return fallback
            else:
                self.logger.warning("Materials v2 resolution already at minimum; disabling")
                return None
        ...
        # Segmentation errors: switch to heuristic backend
            ...
            if config.get("backend") != "heuristic":
                ...
                self.logger.warning(f"Materials v2 backend fallback: {config.get('backend')} → heuristic")
                return fallback
            else:
                self.logger.warning("Materials v2 already using heuristic backend; disabling")
                return None
        ...
        # Confidence errors: lower threshold
            ...
            self.logger.warning(f"Materials v2 confidence fallback: {current_threshold} → {new_threshold}")
            return fallback

        # Unknown error: disable Materials v2 as last resort
        else:
            self.logger.warning(f"Materials v2 unknown error ({error}); disabling as last resort")
            return None
