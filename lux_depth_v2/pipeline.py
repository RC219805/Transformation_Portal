...
class LuxPipelineV2:
    ...
    def process_one(self, img_path: Path, depth_path: Optional[Path] = None) -> Dict[str, object]:
        ...
        with self._stage(report, "io/read_depth"):
            ...
            depth01 = None
            depth_provenance = {}  # Week 1: Depth provenance tracking
            ...
            elif cfg.depth.mode == "auto" or str(cfg.depth.mode).lower() == "auto":
                ...
                try:
                    ...
                    if depth01 is None:
                        ...
                        estimator = create_tiled_estimator(
                            ...
                            overlap=cfg.depth.auto_overlap,
                            device=str(self.device),
                        ...
                        self.logger.info(
                            f"✓ Depth autogenerated: {runtime_ms:.1f}ms, "
                            f"confidence={confidence_proxy:.3f if confidence_proxy else 'N/A'}"
                        )
            ...
            elif cfg.depth.mode == "required" or str(cfg.depth.mode).lower() == "required":
        ...
        if self.materials_v2_engine is not None:
            with self._stage(report, "material/materials_v2"):
                try:
                    # Generate task ID for caching (INCLUDE CONFIG FINGERPRINT)
