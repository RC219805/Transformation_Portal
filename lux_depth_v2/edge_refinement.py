...
def guided_filter_depth(
    ...
    **Security**:
    ...
    if depth_map.shape != rgb_image.shape[:2]:
        raise ValueError(
            f"Shape mismatch: depth {depth_map.shape} vs rgb {rgb_image.shape[:2]}"
        )
...
def gradient_smoothness(
    ...
    **Security**:
    ...
    smoothed = bilateral_depth_filter(
        depth_float,
        d=9,
        sigma_color=50.0,
        sigma_space=50.0
    ...
    result = (
        smoothed * (1.0 - edge_mask * gradient_weight) +
        depth_float * (edge_mask * gradient_weight)
    )
...
def segment_aware_refine(
    ...
    **Security**:
    ...
    if depth_map.shape != segmentation_mask.shape:
        raise ValueError(
            f"Shape mismatch: depth {depth_map.shape} vs mask {segmentation_mask.shape}"
        )
    ...
    for seg_id in segment_ids:
        ...
        blended = (
            depth_region * (1.0 - mask_region_float) +
            smoothed_region * mask_region_float
        )
