...
def test_tiling_detail_fusion(rgb: np.ndarray) -> np.ndarray:
    """Test 3: Global edges + tiled detail fusion (no refinement, no edge snap)."""
    from lux_depth_v2.depth_inference import TiledDepthEstimator, TiledInferenceConfig

    logger.info("\n" + "=" * 60)
    logger.info("TEST 3: Tiling + global edges + tiled detail (detail_fusion)")
    logger.info("=" * 60)
    ...
    config = TiledInferenceConfig(
        ...
        global_fusion_mode="detail_fusion",  # NEW ARCHITECTURE
    ...
    logger.info(f"✓ Detail-fused depth: {depth.shape}")
    return depth
...
def test_edge_snap_only(rgb: np.ndarray) -> np.ndarray:
    ...
    config = DepthRefinementConfig(use_clahe=False, use_edge_filter=False, use_edge_snap=True)
...
def run_isolation_tests(rgb: np.ndarray, output_dir: Path):
    ...
    tests = [
        ...
        ("tiling_detail_fusion", test_tiling_detail_fusion),
    ...
    for test_name, metrics in results.items():
        if "error" in metrics:
            ...
            logger.info(
                f"{test_name:25s} edges={edge_ratio:6.1f}x  overlap={metrics['edge_overlap']:5.1%}  corr={metrics['edge_alignment_correlation']:+.3f}"
            )
