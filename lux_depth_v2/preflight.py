...
class PreFlightValidator:
    ...
    def validate_input_file(
        ...
        try:
            ...
            return ValidationResult(passed=False, message=f"Failed to read image: {e}", severity="error")

    def validate_depth_map(self, input_path: Path, depth_dir: Optional[Path]) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(passed=False, message=f"Failed to read depth map: {e}", severity="error")

    def validate_materials_v2_config(self, config: Optional[Dict[str, any]]) -> ValidationResult:
        ...
        if max_seg_side < min_seg_side:
            issues.append(f"Invalid segmentation sizes: max={max_seg_side} < min={min_seg_side}")
    ...
    def validate_all(
        ...
        report = ValidationReport(passed=passed, results=results, timestamp=time.time())
