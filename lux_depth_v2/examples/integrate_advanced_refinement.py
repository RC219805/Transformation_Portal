...
class EnhancedDepthPipeline:
    ...
    def __init__(
        ...
        logger.info(
            f"Enhanced pipeline initialized: preset={preset}, "
            f"refinement={use_advanced_refinement} ({refinement_technique})"
        )
    ...
    def process_image(
        ...
        if self.use_advanced_refinement:
            ...
            depth_refined = self.refiner.refine(
                depth_raw,
                rgb,
                technique=self.refinement_technique
            )
        else:
        ...
        if compute_metrics:
            if self.use_advanced_refinement:
                ...
                metrics = {
                    'after': compute_edge_metrics(depth_refined, rgb, "comprehensive")
                }
...
def example_batch_processing():
    ...
    if results and results[0]['metrics']:
        avg_f1_before = np.mean([
            r['metrics']['before']['edge_f1'] for r in results
        ])
        avg_f1_after = np.mean([
            r['metrics']['after']['edge_f1'] for r in results
        ])
