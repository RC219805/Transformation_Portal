...
class ProcessOrchestrator:
    ...
    def __init__(
        ...
        self.logger.info(
            f"ProcessOrchestrator initialized | workers={max_workers} memory_budget={memory_budget_gb}GB device={device}"
        )
    ...
    def submit_task(self, task_config: TaskConfig, priority: int = 0) -> str:
        ...
        self.logger.debug(f"Task submitted | id={task_config.task_id} priority={priority} input={task_config.input_path.name}")
    ...
    def _start_worker(
        ...
        self.logger.info(
            f"Worker started | task_id={task_config.task_id} pid={process.pid} input={task_config.input_path.name}"
        )
        ...
        for task_id, process in self.active_workers.items():
            if not process.is_alive():
                ...
                if exit_code == 0:
                    ...
                    self.logger.error(f"Worker failed | task_id={task_id} exit_code={exit_code}")
...
def _worker_task(task_config: TaskConfig, device: str, logger):
    ...
    try:
        ...
        logger.info(f"Worker complete | task_id={task_config.task_id} status={result.get('status')}")
...
class ParallelOrchestrator(ProcessOrchestrator):
    ...
    def check_parallel_capacity(self, required_workers: int) -> ParallelCapacityCheck:
        ...
        try:
            ...
            if recommended_workers < required_workers:
                warnings.append(f"Insufficient memory for {required_workers} workers. Recommended: {recommended_workers}")
            if available_disk_gb < 20.0:
                warnings.append(f"Low disk space: {available_disk_gb:.1f}GB available")
