...
class ResourceMonitor:
    ...
    def is_safe_to_process(self, image_size_mp: float, upscale: int = 4, strict: bool = False) -> bool:
        ...
        if metrics.ram_percent > ram_threshold:
            self.logger.warning(f"High RAM usage | current={metrics.ram_percent:.1f}% threshold={ram_threshold:.1f}%")
            return False
    ...
    def check_parallel_capacity(self, required_workers: int, memory_per_worker_gb: float) -> Dict[str, Any]:
        ...
        if metrics.ram_percent > 80.0:
            warnings.append(f"High RAM usage: {metrics.ram_percent:.1f}%")
