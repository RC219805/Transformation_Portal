...
class PerformanceReport:
    ...
    def finalize(self):
        ...
        if self.total_time > 0:
            ...
            self.bottlenecks = [(s.name, s.elapsed_time) for s in self.stages if s.elapsed_time > threshold]
...
class PerformanceProfiler:
    ...
    def stage(self, name: str, is_io: bool = False, **metadata):
        ...
        try:
            ...
            self.logger.debug(
                f"Stage completed: {name} | elapsed={metrics.elapsed_time:.3f}s memory_delta={metrics.memory_delta_mb:.1f}MB"
            )
    ...
    def io_operation(self, operation: str):
        ...
        try:
            ...
            if self.current_stage:
                ...
                self.current_stage.compute_time = max(0, self.current_stage.compute_time - elapsed)

                if "io_operations" not in self.current_stage.metadata:
                    self.current_stage.metadata["io_operations"] = []

                self.current_stage.metadata["io_operations"].append({"operation": operation, "time": round(elapsed, 3)})
...
def analyze_batch_performance(reports: List[PerformanceReport]) -> Dict:
    ...
    common_bottlenecks = [
        (name, count, 100 * count / len(reports)) for name, count in sorted(bottleneck_counts.items(), key=lambda x: -x[1])
    ]
    ...
    return {
        ...
        "common_bottlenecks": [
            {"stage": name, "count": count, "percent": round(pct, 1)} for name, count, pct in common_bottlenecks
        ],
    }
