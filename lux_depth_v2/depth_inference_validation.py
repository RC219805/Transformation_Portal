...
class TiledDepthEstimator:
    ...
    def __init__(self, config: TiledInferenceConfig):
        ...
        logger.info(
            f"  Global anchor: {self.config.use_global_anchor} (mode={self.config.global_fusion_mode}), Edge snapping: {self.config.use_edge_snapping}"
        )
    ...
    def _validate_boundary_energy(self, depth: np.ndarray, tile_coords: Optional[Tuple[int, int, int, int]] = None) -> None:
        """
    ...
    def _infer_tile(self, tile_rgb: np.ndarray) -> np.ndarray:
        ...
        if depth.shape != tile_rgb.shape[:2]:
            ...
            depth = np.array(PILImage.fromarray(depth).resize((tile_rgb.shape[1], tile_rgb.shape[0]), PILImage.LANCZOS))
    ...
    def estimate_depth(self, image: np.ndarray, rgb_for_fusion: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        need_global = bool(self.config.reconcile_scales) or (
            bool(self.config.use_global_anchor) and fusion_mode in ("detail_fusion", "legacy")
        )
        if need_global:
