...
def apply_stone_response(
    ...
    if rgb01.shape[:2] != stone_mask.shape:
        raise ValueError(f"Image shape {rgb01.shape[:2]} != mask shape {stone_mask.shape}")
    ...
    if total_px < cfg.min_coverage_px:
        log.info(f"Stone coverage {total_px}px < min {cfg.min_coverage_px}px, skipping")
        return rgb01, {
            "applied": False,
    ...
    log.debug(f"Stone zones: core={core_px}px, edge={edge_px}px, total={total_px}px")
    ...
    if core_px > 0:
        ...
        if clamp_count > 0:
            log.warning(f"{clamp_count} core pixels exceeded max_delta={cfg.max_delta:.3f}; clamping")
    ...
    if edge_px > 0:
        ...
        if edge_clamp_count > 0:
            log.warning(f"{edge_clamp_count} edge pixels exceeded max_delta={cfg.max_delta:.3f}; clamping")
            for i in np.where(excessive)[0]:
                scale_back = cfg.max_delta / per_pixel_max_delta[i]
    ...
    if edge_delta_p95 > cfg.halo_p95_threshold:
        halo_risk = "HIGH"
        log.warning(f"Halo risk: edge p95 delta {edge_delta_p95:.4f} > threshold {cfg.halo_p95_threshold:.4f}")
    ...
    log.info(f"Stone response applied: {total_px}px, mean_delta={mean_delta:.4f}, halo_risk={halo_risk}")
