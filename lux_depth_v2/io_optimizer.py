...
class AsyncTIFFWriter:
    ...
    def __init__(self, use_compression: bool = True, compression: str = "lzw", max_workers: int = 2):
        ...
        logger.info(f"AsyncTIFFWriter initialized: compression={self.compression}, workers={max_workers}")
    ...
    def write_tiff_background(
        ...
        """
        future = self.executor.submit(self._write_tiff_sync, image, path, metadata)
    ...
    def _write_tiff_sync(self, image: np.ndarray, path: Path, metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """
...
class StreamingUpscaleWriter:
    ...
    def write_tile(self, tile: np.ndarray, position: tuple, weight: Optional[np.ndarray] = None):
        """
        ...
        if x < 0 or y < 0 or x + w > self.width or y + h > self.height:
            logger.warning(
                f"Tile {self.tiles_written} out of bounds: pos=({x},{y}), size=({w}×{h}), image=({self.width}×{self.height})"
            )
        ...
        if self.tiles_written % 10 == 0:
            ...
            logger.debug(f"Tiles written: {self.tiles_written}, elapsed: {elapsed:.1f}s")
    ...
    def finalize(self) -> Path:
        ...
        Returns:
            Output file path
        """
        logger.info(f"Finalizing {self.tiles_written} tiles to {self.output_path.name}...")
...
def create_streaming_writer(
    output_path: Path, final_dimensions: tuple, compression: Optional[str] = None
