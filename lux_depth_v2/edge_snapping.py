...
class EdgeSnapper:
    ...
    def _detect_rgb_edges(self, rgb: np.ndarray) -> np.ndarray:
        ...
        edges = cv2.Canny(gray, self.config.edge_threshold_low, self.config.edge_threshold_high)

        return edges.astype(np.float32) / 255.0

    def _joint_bilateral_filter(self, depth: np.ndarray, rgb: np.ndarray) -> np.ndarray:
        """
    ...
    def _snap_at_edges(self, depth: np.ndarray, depth_filtered: np.ndarray, edges: np.ndarray) -> np.ndarray:
        """
        ...
        snapped = snap_weight * depth_filtered + (1 - snap_weight) * depth

        return snapped

    def snap(self, depth: np.ndarray, rgb: np.ndarray) -> np.ndarray:
        """
    ...
    def snap_multiscale(self, depth: np.ndarray, rgb: np.ndarray) -> np.ndarray:
        """
