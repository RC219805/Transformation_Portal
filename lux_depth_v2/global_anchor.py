...
class GlobalAnchorFusion:
    ...
    def __init__(self, config: GlobalAnchorConfig):
        self.config = config
        logger.info(
            f"GlobalAnchorFusion: global_size={config.global_max_size} weights={config.global_weight:.2f}/{config.tile_weight:.2f}"
        )
    ...
    def _upsample_global_depth(self, global_depth: np.ndarray, target_shape: Tuple[int, int]) -> np.ndarray:
    ...
    def fuse(
        ...
        if self.config.use_frequency_split:
            ...
            fused = self.config.global_weight * global_depth + self.config.tile_weight * tiled_depth
        ...
        if self.config.edge_aware_fusion and rgb is not None:
            ...
            fused_refined = edge_weights * tiled_depth + (1 - edge_weights) * fused
