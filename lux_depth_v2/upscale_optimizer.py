...
class TileBasedUpscaler:
    ...
    def __init__(
        ...
        logger.info(
            f"TileBasedUpscaler initialized: backend={backend}, tile_size={tile_size}, overlap={overlap}, device={device}"
        )
    ...
    def upscale_progressive(
        ...
        logger.info(f"Progressive upscale: {w}×{h} → {final_w}×{final_h} ({scale_factor}×, progressive={progressive})")
    ...
    def _create_weight_map(self, shape: Tuple[int, int], overlap: int) -> np.ndarray:
        """
...
def upscale_progressive(
    ...
    Returns:
        Output path
    """
    upscaler = TileBasedUpscaler(backend=backend, tile_size=tile_size)
