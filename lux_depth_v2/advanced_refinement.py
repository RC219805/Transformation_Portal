...
class DepthRefiner:
    ...
    def _validate_opencv(self) -> None:
        ...
        if not hasattr(cv2, 'ximgproc'):
            logger.warning(
                "cv2.ximgproc not available. Guided filter will fallback to bilateral."
            )
    ...
    def bilateral_filter(
        ...
        logger.info(
            f"Bilateral filter applied: d={d}, σ_color={sigma_color}, "
    ...
    def edge_guided_enhancement(
        ...
        logger.info(
            f"Edge-guided enhancement: {edge_pixel_count} edge pixels, "
            f"σ={blur_sigma}"
        )
    ...
    def gradient_consistency_filter(
        ...
        logger.info(
            f"Gradient consistency: {smooth_pixel_count} smoothed pixels, "
            f"threshold={threshold:.4f}"
        )
        
        return self._denormalize_depth(filtered, metadata)
    
    def hybrid_refinement(
        self,
        depth: np.ndarray,
        rgb: np.ndarray
    ) -> np.ndarray:
    ...
    def refine(
        ...
        if rgb is None:
            logger.warning(
                f"Technique '{technique}' requires RGB image. "
                f"Falling back to bilateral filter."
            )
            return self.bilateral_filter(depth)
...
def compute_chamfer_distance(
    ...
    chamfer = (
        np.percentile(distances_pred_to_gt, percentile) + 
        np.percentile(distances_gt_to_pred, percentile)
    ) / 2.0
