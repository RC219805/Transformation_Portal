...
def apply_glass_response(
    ...
    if rgb01.shape[:2] != glass_mask.shape:
        raise ValueError(f"Image shape {rgb01.shape[:2]} != mask shape {glass_mask.shape}")
    ...
    log.debug(f"Glass zones: core={core_px}px, edge={edge_px}px, blend={blend_px}px")
    ...
    if core_px > 0:
        ...
        if excessive.any():
            log.warning(f"{excessive.sum()} core pixels exceeded max_delta; clamping")
    ...
    if edge_px > 0:
        ...
        excessive = delta > cfg.max_delta
        if excessive.any():
            log.warning(f"{excessive.sum()} edge pixels exceeded max_delta; clamping")
    ...
    if blend_px > 0:
        ...
        result[blend_mask] = alpha * rgb01[blend_mask] + (1 - alpha) * result[blend_mask]
