...
def compare_pipelines(rgb: np.ndarray, output_dir: Path) -> dict:
    """
    ...
    improvements = {
        "edge_gradient_mean": (production_metrics.edge_gradient_mean / max(baseline_metrics.edge_gradient_mean, 0.01) - 1)
        * 100,
    ...
    logger.info(
        f"  Edge gradient (mean): {production_metrics.edge_gradient_mean:.2f} ({improvements['edge_gradient_mean']:+.1f}%)"
    )
    logger.info(
        f"  Edge gradient (p95):  {production_metrics.edge_gradient_p95:.2f} ({improvements['edge_gradient_p95']:+.1f}%)"
    )
    logger.info(
        f"  Edge alignment:       {production_metrics.edge_alignment_rgb:.3f} ({improvements['edge_alignment']:+.1f} pp)"
    )
