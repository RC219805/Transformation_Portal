...
def validate_tile_inference(
    ...
    # Results
    results = {"test_image_size": test_image_size, "tile_tests": []}
    ...
    for tile_size in tile_sizes:
        ...
        try:
            ...
            results["tile_tests"].append({"tile_size": tile_size, "error": str(e)})

    # Overall verdict
    all_passed = all(t.get("verdict", "").startswith("PASS") for t in results["tile_tests"] if "verdict" in t)

    results["overall_verdict"] = (
        "PASS: Tiled inference is genuinely high-res" if all_passed else "FAIL: Internal resizing detected"
    )
