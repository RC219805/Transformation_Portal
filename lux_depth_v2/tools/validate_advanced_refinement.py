...
def compute_pass_rate(results: List[Dict], metric_name: str = "edge_f1", threshold: float = 0.55) -> Tuple[float, float]:
    """
    ...
    """
    pass_count_before = sum(1 for r in results if r["metrics_before"].get(metric_name, 0) >= threshold)
    pass_count_after = sum(1 for r in results if r["metrics_after"].get(metric_name, 0) >= threshold)
...
def generate_report(results: List[Dict], output_path: Path) -> None:
    ...
    for metric_name in ["edge_f1", "edge_alignment", "edge_precision", "edge_recall"]:
        if metric_name in results[0]["metrics_before"]:
            ...
            report["summary"][f"{metric_name}_improvement_pct"] = (pass_after - pass_before) * 100
    ...
    for i, result in enumerate(results):
        scene_report = {
            "scene_id": i,
            "metrics_before": {
                k: float(v) if isinstance(v, (int, float, np.number)) else v for k, v in result["metrics_before"].items()
            },
            "metrics_after": {
                k: float(v) if isinstance(v, (int, float, np.number)) else v for k, v in result["metrics_after"].items()
            },
            "improvements": {
                k: float(v) if isinstance(v, (int, float, np.number)) else v for k, v in result["improvements"].items()
            },
        ...
        if result["gt_metrics"]:
            scene_report["gt_metrics"] = {
                k: float(v) if isinstance(v, (int, float, np.number)) else v for k, v in result["gt_metrics"].items()
            }
...
def main():
    ...
    parser.add_argument("--config", type=Path, help="JSON config file for AdvancedRefinementConfig")
