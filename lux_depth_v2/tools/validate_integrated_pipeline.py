...
def compute_metrics(depth: np.ndarray, rgb: np.ndarray) -> dict:
    ...
    return {**edge_metrics, **depth_stats}
...
def compare_pipelines(rgb: np.ndarray, output_dir: Path):
    ...
    # Save report
    report = {
        "baseline": {**metrics_baseline, "time_seconds": time_baseline},
        "full_pipeline": {**metrics_full, "time_seconds": time_full},
    ...
    logger.info(
        f"  Edge alignment:       {metrics_full.get('edge_alignment', 0):.3f} ({improvements['edge_alignment']:+.1f} pp)"
    )
