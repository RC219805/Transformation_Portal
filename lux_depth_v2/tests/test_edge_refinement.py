...
class TestEnhanceEdgesWithGuidance:
    ...
    def test_edge_sharpening(self, synthetic_depth, synthetic_rgb):
        ...
        enhanced = enhance_edges_with_guidance(
            synthetic_depth, synthetic_rgb, strength=0.5, threshold=30.0
        )
...
class TestConvenienceFunctions:
    ...
    def test_refine_depth_edge_aware_preset(self, simple_depth, simple_rgb):
        ...
        subtle = refine_depth_edge_aware(
            simple_depth, simple_rgb, preset=RefinementPreset.SUBTLE
        )
        aggressive = refine_depth_edge_aware(
            simple_depth, simple_rgb, preset=RefinementPreset.AGGRESSIVE
        )
    ...
    def test_refine_depth_edge_aware_with_segments(self, simple_depth, simple_rgb):
        ...
        refined = refine_depth_edge_aware(
            simple_depth, simple_rgb, segmentation_mask=segments
        )
...
class TestIntegration:
    ...
    def test_full_workflow_noisy_depth(self, noisy_depth, synthetic_rgb):
        ...
        pipeline = EdgeRefinementPipeline(
            EdgeRefinementConfig.from_preset(RefinementPreset.BALANCED)
        )
