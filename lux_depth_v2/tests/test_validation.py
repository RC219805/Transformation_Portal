...
def test_create_synthetic_pair():
    ...
    degraded, reference = degradation.create_synthetic_pair(original, degradations=["downsample", "noise"])
...
def test_compute_composite_score():
    ...
    metric_scores = {"ssim": 0.95, "psnr": 40.0, "lpips": 0.10, "nima": 8.0}

    weights = {"ssim": 0.25, "psnr": 0.15, "lpips": 0.35, "nima": 0.25}
...
def test_validate_batch_mode(mode, tmp_path):
    ...
    with patch.object(validator, "_load_image") as mock_load:
        ...
        with patch.object(validator, "_find_reference") as mock_find_ref:
            ...
            report = validator.validate_batch(test_images=test_images, output_dir=tmp_path / "validation", mode=mode)
