...
def compute_ssim(img1: np.ndarray, img2: np.ndarray, multichannel: bool = True) -> float:
    ...
    try:
        ...
        if img1.ndim == 3 and multichannel:
            ...
            return float(
                np.mean([compute_ssim(img1[..., i], img2[..., i], multichannel=False) for i in range(img1.shape[-1])])
            )
...
def _heuristic_aesthetic_score(img: np.ndarray) -> float:
    ...
    score = dr * 3.0 + color_balance * 2.0 + min(sharpness * 10, 3.0) + min(contrast * 5, 2.0)

    # Scale to [1, 10]
    return float(np.clip(score, 1.0, 10.0))


def compute_all_metrics(img: np.ndarray, reference: Optional[np.ndarray] = None, device: str = "cpu") -> dict:
