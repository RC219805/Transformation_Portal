...
class QualityValidator:
    ...
    def __init__(self, device: str = "cpu", default_weights: Optional[Dict[str, float]] = None):
    ...
    def validate_batch(
        ...
        for test_path in test_images:
            ...
            # Compute metrics
            img_metrics = metrics.compute_all_metrics(test_img, reference=reference_img, device=self.device)
        ...
        report.composite_score = self._compute_composite_score(report.metrics_scores, weights)

        # Baseline comparison if provided
        if baseline_dir:
            report.baseline_comparison = self._compare_with_baseline(
                test_images, baseline_dir, mode=mode, metrics_list=metrics_list
            )

        return report

    def create_synthetic_reference(
        self, original: Path, output_dir: Path, degradations: Optional[List[str]] = None
    ) -> Tuple[Path, Path]:
        ...
        degraded, reference = degradation.create_synthetic_pair(original_img, degradations=degradations)
    ...
    def compare_baselines(
        ...
        our_metrics = metrics.compute_all_metrics(our_img, reference=reference_img, device=self.device)
        baseline_metrics = metrics.compute_all_metrics(baseline_img, reference=reference_img, device=self.device)
    ...
    def _compute_composite_score(self, metric_scores: Dict[str, float], weights: Dict[str, float]) -> float:
    ...
    def _compare_metrics(self, ours: Dict[str, float], baseline: Dict[str, float]) -> int:
