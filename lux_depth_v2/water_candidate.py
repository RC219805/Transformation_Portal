...
class WaterCandidateDetector:
    ...
    def detect(
        ...
        combined_mask = self._combine_cues(chroma_mask, specular_mask, texture_mask, planarity_mask)
    ...
    def _chromaticity_cue(self, hsv: np.ndarray, scene_context: SceneContext) -> Tuple[np.ndarray, float]:
    ...
    def _specular_cue(self, rgb01: np.ndarray, hsv: np.ndarray) -> Tuple[np.ndarray, float]:
        """High highlights + low saturation pockets (water reflections)."""
        val = hsv[:, :, 2]
        sat = hsv[:, :, 1]

        # High value (bright) with low saturation (specular reflection)
        specular = (val >= self.params.specular_highlight_threshold) & (sat <= self.params.specular_low_sat_threshold)
    ...
    def _postprocess(self, mask: np.ndarray) -> np.ndarray:
        ...
        if self.params.morph_close_kernel > 0:
            binary_mask = binary_closing(binary_mask, disk(self.params.morph_close_kernel))

        # Morphological open (remove noise)
        if self.params.morph_open_kernel > 0:
            binary_mask = binary_opening(binary_mask, disk(self.params.morph_open_kernel))
    ...
    def _component_filtering(self, mask: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        component_sizes = scipy_ndimage.sum(mask, labeled, range(1, num_features + 1))
    ...
    def _apply_suppressors(
        ...
        if self.params.flat_surface_suppressor_enabled:
            is_flat_surface, flat_metrics = self._detect_flat_blue_surface(rgb01, hsv, final_mask, specular_mask)
    ...
    def _detect_flat_blue_surface(
        ...
        is_flat = (
            total_edge_energy < self.params.flat_surface_edge_energy_threshold
            and specular_fraction < self.params.flat_surface_specular_fraction_threshold
        )
        ...
        if self.params.glass_multiscale_enabled and grid_score > 0.0:
            ...
            if grid_score_coarse is not None:
                ...
                tile_exempted = (
                    ...
                    and grid_persistence_ratio < self.params.glass_tile_persistence_threshold
                )

        # Detection logic: high axis-alignment or grid pattern, minus tile exemptions
        is_glass_base = (
            alignment_score > self.params.glass_edge_alignment_threshold or grid_score > self.params.glass_grid_score_threshold
        )
