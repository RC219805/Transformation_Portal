...
class NormalMapGenerator:
    ...
    def generate(self, depth: np.ndarray, strength: float = 1.0) -> np.ndarray:
        """
    ...
    def validate_normal_map(self, normals: np.ndarray) -> Dict[str, float]:
        ...
        logger.info(
            f"Normal map validation: X_std={nx_std:.3f} Y_std={ny_std:.3f} Z_mean={nz_mean:.3f} angle_med={angle_median:.1f}°"
        )
...
def generate_normal_map(depth: np.ndarray, preset: str = "architectural", strength: float = 1.0) -> np.ndarray:
    """
