...
class ConfidenceMetrics:
    ...
    def is_high_quality(self, threshold: float = 0.6) -> bool:
        ...
        return self.confidence_avg >= threshold and self.high_confidence_pct >= 0.7
...
def calculate_segmentation_size(original_size: Tuple[int, int], config: SegmentationConfig) -> Tuple[int, int]:
...
def create_soft_mask(mask: np.ndarray, config: SegmentationConfig) -> np.ndarray:
...
def generate_confidence_mask(confidence_map: np.ndarray, material_type: str, config: ConfidenceConfig) -> np.ndarray:
    ...
    if config.blend_mode == "soft":
        ...
        mask = np.clip((confidence_map - (threshold - blend_range)) / blend_range, 0.0, 1.0)
    else:  # 'hard'
...
class MaterialsV2Engine:
    ...
    def __init__(self, config: MaterialsV2Config, device: str = "auto", logger=None):
        self.config = config
        self.device = device
    ...
    def segment_with_confidence(self, image: np.ndarray, task_id: Optional[str] = None) -> SegmentationResult:
        ...
        seg_size = calculate_segmentation_size(original_size, self.config.segmentation)
        ...
        if needs_downscale:
            ...
            self.logger.debug(f"Downscaled for segmentation: {original_size} → {seg_size}")
        else:
            image_seg = image
        ...
        if needs_downscale:
            ...
            masks = {k: create_soft_mask(v, self.config.segmentation) for k, v in masks.items()}
            upsampled = True
        else:
            masks = masks_seg
    ...
    def apply_gated_response(
        ...
        for material_type, mask in segmentation.masks.items():
            ...
            gated_mask = generate_confidence_mask(confidence_map, material_type, self.config.confidence)
    ...
    def _segment_image(self, image: np.ndarray) -> Tuple[Dict, Dict]:
        ...
        try:
            ...
            masks_np = {k: v.cpu().numpy().squeeze() if hasattr(v, "cpu") else v for k, v in masks.items()}
    ...
    def _upsample_masks(self, masks: Dict[str, np.ndarray], target_size: Tuple[int, int]) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            for material_type, mask in masks.items():
                if mask.ndim == 2:
                    upsampled[material_type] = cv2.resize(mask, (w, h), interpolation=cv2.INTER_CUBIC)
                else:
                    # Multi-channel mask
                    upsampled[material_type] = cv2.resize(mask, (w, h), interpolation=cv2.INTER_CUBIC)
            return upsampled
    ...
    def _calculate_metrics(self, masks: Dict[str, np.ndarray], confidences: Dict[str, np.ndarray]) -> ConfidenceMetrics:
        """Calculate confidence metrics."""
    ...
    def _load_from_cache(self, task_id: str, image: np.ndarray) -> Optional[SegmentationResult]:
        ...
        if self._cache_manager is None:
            ...
            self._cache_manager = MaskCacheManager(Path(self.config.cache_dir), logger=self.logger)
    ...
    def _save_to_cache(self, task_id: str, image: np.ndarray, result: SegmentationResult):
...
class CLIPMaterialClassifier:
    ...
    def query_natural_language(self, rgb: "torch_ops.torch.Tensor", query: str) -> "torch_ops.torch.Tensor":
