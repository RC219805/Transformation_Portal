...
|----------|----------|-------------------|
| macOS Apple Silicon (`darwin-arm64`) | `ml-core-darwin-arm64.txt` | `torch==2.13.0` + `torchvision==0.28.0` + `diffusers>=0.38.0` + `transformers==5.10.4` + pinned `coremltools` |

**Contract notes:**
- Supported target-owned core locks anchor on torch 2.13.0 / torchvision 0.28.0, Diffusers 0.38.0+, and Transformers `>=5.10.1,<5.11`.
...
| Starlette | `requirements/base.in` + `pyproject.toml` bound | `1.6.0` |
...
|-------|----------|-----------------|-------|
| ml-core | Supported PyTorch baseline (torch 2.13.0, torchvision 0.28.0, Transformers 5.10.x) | darwin-arm64 | Base ML functionality |
