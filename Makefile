...
help:
	...
	@echo "  test-accessibility-browser  Run axe, contrast, reflow, adaptive-mode, and hydrated interaction checks"
...
.PHONY: test-lux-depth-contract
test-lux-depth-contract:
	@PYTHONPATH=src "$(PY)" -m pytest -q tests/lux_depth tests/lux_depth_v6 tests/lux_depth_v5 tests/lux_depth_v4 tests/core/test_depth_evidence.py tests/core/test_execution_plan_v2.py tests/materials_v4 tests/test_audit_lux_depth_v6_reconstruction.py
...
test-lux-depth-v5-managed-contract:
	@PYTHONPATH=src "$(PY)" -m pytest -q tests/orchestrator/test_photography_adapter.py tests/orchestrator/test_job_execution_service.py tests/orchestrator/test_managed_photography_api.py tests/orchestrator/test_managed_photography_publisher.py tests/orchestrator/test_operational_storage_offline.py tests/orchestrator/test_execution_dispatch.py tests/orchestrator/test_worker_process_contract.py

# The fixtures truncate only a dedicated migrated *_test database.
test-lux-depth-v5-managed-services:
	@if [ -z "$(TP_DISPATCH_TEST_DATABASE_URL)" ] || [ -z "$(TP_DISPATCH_TEST_REDIS_URL)" ]; then \
		echo "ERROR: set TP_DISPATCH_TEST_DATABASE_URL to a dedicated migrated *_test database and TP_DISPATCH_TEST_REDIS_URL."; \
		exit 1; \
	fi
	@PYTHONPATH=src TP_DISPATCH_TEST_DATABASE_URL="$(TP_DISPATCH_TEST_DATABASE_URL)" TP_PHOTOGRAPHY_TEST_DATABASE_URL="$(TP_DISPATCH_TEST_DATABASE_URL)" TP_DISPATCH_TEST_REDIS_URL="$(TP_DISPATCH_TEST_REDIS_URL)" "$(PY)" -m pytest -q tests/orchestrator/test_photography_storage_services.py tests/orchestrator/test_managed_photography_services.py
...
test-accessibility-browser:
	@echo "Running frontdoor and Portal accessibility plus hydrated interaction browser checks..."
	@./scripts/setup/ensure_node_version.sh
	@cd web/secure-landing && npm run test:browser:a11y
