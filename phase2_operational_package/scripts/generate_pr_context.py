...
def load_knowledge_base(cache_dir: Path) -> Dict:
    ...
    knowledge = {"test_results": [], "quality_metrics": [], "detected_patterns": {}}
...
def analyze_changed_files(changed_files: List[str], knowledge: Dict) -> Dict:
    ...
    pass_rate_metrics = [m for m in quality_metrics if m.get("metric_id") == "test_pass_rate"][-5:]
    ...
    if analysis["failure_history"]:
        analysis["recommendations"].append("Pay extra attention to tests with historical failures")

    if analysis["quality_trend"] and analysis["quality_trend"]["direction"] == "declining":
        analysis["recommendations"].append("Quality trend is declining - consider adding more tests")

    return analysis


def generate_context_markdown(changed_files: List[str], analysis: Dict) -> str:
...
def main():
    parser = argparse.ArgumentParser(description="Generate PR context from Knowledge Engine")
