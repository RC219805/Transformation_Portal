...
> Historical note: this document is retained as point-in-time review evidence.
> The maintained operator entrypoint is `tools/ad_editorial_post_pipeline.py`;
> deprecated v2/v3 snapshots live under `tools/deprecated/`.
...
7. ✅ Edit the generated project `DOCS` directory's `selects.csv` file to cull
   unwanted images
