...
- CI bootstrap tools are part of the security baseline. Determinism workflows pin `pip==26.2.1`, `setuptools==83.0.0`, and `wheel==0.46.2`; lock-generation workflows pair pip 26.2.1 with `pip-tools==7.6.1`, the first compiler release compatible with pip 26.2. The requirements Makefile rejects any other paired pip or pip-tools version before lock generation. These pins must move atomically when a bootstrap-tool advisory or compatibility boundary changes. Pip 26.2.1 is the governed baseline because it contains the CVE-2026-13346 fix from 26.2 plus the follow-up keyring fix. PyPI records Trusted Publishing provenance for both pip 26.2.1 and pip-tools 7.6.1.
