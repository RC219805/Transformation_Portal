...
- CI bootstrap tools are part of the security baseline. Determinism workflows pin `pip==26.1.2`, `setuptools==83.0.0`, and `wheel==0.46.2`; lock-generation workflows pair pip 26.1.2 with `pip-tools==7.6.0`, the first release with upstream pip 26.1 support. These pins must move atomically when a bootstrap-tool advisory or compatibility boundary changes.
