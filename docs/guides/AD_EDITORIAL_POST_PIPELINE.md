...
**Current maintained entrypoint:** `tools/ad_editorial_post_pipeline.py`

Deprecated v2/v3 snapshots are retained under `tools/deprecated/` for
historical reference only. Do not use them as operator entrypoints for current
workflows.
...
- **Multiple Styles:** Natural, Minimal, Cinematic, plus configured custom styles
...
See `tools/sample_config.yml` for comprehensive documentation of all settings.
...
2. **Select Images**
   - Edit `selects.csv` in the generated project `DOCS` directory to mark keepers
...
Create `metadata.csv` in the generated project `DOCS` directory:
...
### Current Maintained Tool
- `tools/ad_editorial_post_pipeline.py`
- Critical bug fixes, 16-bit TIFF preservation, config validation, and atomic
  file writes
- Serial processing; multiprocessing and resume remain outside the maintained
  active entrypoint

### Historical Snapshots
- `tools/deprecated/ad_editorial_post_pipeline_v2.py`
- `tools/deprecated/ad_editorial_post_pipeline_v3.py`
- `tools/deprecated/test_ad_pipeline.py`
- `tools/deprecated/test_ad_pipeline_v3.py`

Historical implementation notes live in
`docs/historical/script-audits/AD_EDITORIAL_POST_PIPELINE_FIXES_APPLIED.md`,
`docs/historical/script-audits/AD_EDITORIAL_POST_PIPELINE_V2_ENHANCEMENTS.md`,
and
`docs/historical/script-audits/AD_EDITORIAL_POST_PIPELINE_IMPLEMENTATION_SUMMARY.md`.
...
For issues or feature requests, start with this README and the maintained
entrypoint above. Use the historical notes only when auditing old snapshots.
