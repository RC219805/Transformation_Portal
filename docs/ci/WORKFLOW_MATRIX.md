...
| 15 | `performance-monitor.yml` | Performance Monitor | schedule (3:30 AM UTC), manual | ❌ No | 232 | **Keep** — schedule-only by design; runs the Lux perf smoke harness against committed baselines (`tests/benchmarks/baselines/`) and is green only when `scripts/ci/check_performance_evidence.py` proves the required tests executed and the four artifacts were produced (repair 1.6-a, #2062); distinct from nightly benchmarks. |
