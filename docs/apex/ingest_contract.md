# Ingest Contract Documentation (v1.0.2)
...
|--------|--------|----------|------------------|
| **ProvenanceSidecar** | JSON (v1.0.2) | Audit trail | Complete EXIF + toolchain + env metadata |
| **IngestManifest** | JSON (v1.0.2) | Pipeline orchestrator | Summary status + sidecar pointer |
...
```json
{
  "schema_version": "1.0.2",  // Literal type (only "1.0.2" accepted)
  "file_integrity": {
...
**Contract:** Schema version must be exactly `"1.0.2"`.

**Enforcement:**
- Literal type in Pydantic model (`Literal["1.0.2"]`)
...
**EXIF normalization semantics (v1.0.2):**
...
This is the initial release (v1.0.2). No migration required.

**Future schema changes:**

When updating schema to v2.0.0:
1. Create new models with Literal["2.0.0"]
2. Keep v1.0.2 models for backward compat
