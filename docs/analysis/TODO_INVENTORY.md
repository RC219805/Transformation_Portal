...
- ✅ **Scanner snapshot regenerated from live repo state.** `python scripts/validation/scan_todo_inventory.py --write-snapshot` refreshed
  [`todo_scanner_snapshot.json`](todo_scanner_snapshot.json) directly from the scanner payload.
- 📊 **Current scanner baseline:** 25 `NotImplementedError` items, 0 ungoverned TODOs, 1,753 files scanned.
- 📊 **Baseline delta:** 24 → 25 governed items. The additional visible item is
  `src/transformation_portal/comfyui/executor.py`, which fails closed for
  unsupported ComfyUI node executor types; no ungoverned backlog item was
  opened by this refresh.
- ✅ **No source-code `TODO|FIXME|HACK|XXX` backlog opened.** The only scanner
  ...
  contracts, phase gates, platform limits, fail-closed dispatch guards, or test
  protocol stubs.
...
- **25 NotImplementedError instances** - Correct stubs (abstract methods, phase gates, platform limitations, fail-closed dispatch guards)
...
**Methodology:** `python scripts/validation/scan_todo_inventory.py --write-snapshot` (2026-06-11)
...
**Categorical breakdown** (as of 2026-06-11 scan):
...
- **Phase gates / feature stubs:** ~5 instances — single-view reconstruction (`spatial_ai/orchestration/pipeline.py`), non-linear scene interpolation guard, unknown-backend guard, ComfyUI unsupported-node executor guard, etc.
...
- **Scripts / one-shots:** ~3 — `scripts/ci/apex/verify_contract.py`, `scripts/pipelines/unified_meta_pipeline.py`, `scripts/utilities/depth_anything_v2.py`

The previous v2.4.0 count of "12" tracked only `src/` and contained internal arithmetic errors (5+4+1≠12). The current 25-item scanner baseline is the full scanner scope and is the figure CI enforces against.

**Priority Recommendation:**
- No source code TODOs requiring action
- Scanner-visible test TODO comments are 0
- All NotImplementedError instances are governed; no governance violations as of 2026-06-11
...
## 2026-06-11 Refresh — Scanner Snapshot And Documentation Alignment

**Trigger:** Documentation forensic alignment pass against live scanner and CI
inventory state.
**Scanner snapshot:** [`todo_scanner_snapshot.json`](todo_scanner_snapshot.json)
regenerated with `python scripts/validation/scan_todo_inventory.py --write-snapshot`.

**Findings:**
- `python scripts/validation/scan_todo_inventory.py --check-governance` → exit 0; **0 ungoverned TODOs** across 1,753 files scanned.
- **25** `NotImplementedError` instances, all governed (abstract methods, phase gates, platform limits, fail-closed dispatch guards, or test protocol stubs).
- **Zero** `TODO|FIXME|XXX|HACK` comment markers in scanner scope.
- The 24 → 25 delta comes from the scanner-visible
  `src/transformation_portal/comfyui/executor.py` unsupported-node dispatch
  guard. That is a deterministic fail-closed behavior, not an ungoverned
  backlog item.

**Action taken:** Refreshed this inventory, the action plan, quick reference,
scanner snapshot, TODO priority schema, documentation index, and documentation
map so current counts, commands, and governance language point at the same live
scanner contract.

**Diff vs prior baseline:** scanner scope grew from 1,570 to 1,753 files because
repo content expanded; governed item count increased by one due to the ComfyUI
executor guard. Governance posture remains unchanged: 0 ungoverned TODOs.

---

**END OF TODO INVENTORY v2.4.8**
