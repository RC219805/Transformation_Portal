    ...
    def generate_report(self) -> Path:
        ...
        with open(report_path, "w") as f:
            ...
            if self.comparisons:
                ...
                if psnr_values:
                    f.write(
                        f"- **Average PSNR:** {np.mean(psnr_values):.2f} dB (range: {min(psnr_values):.2f} - {max(psnr_values):.2f})\n"
                    )
                if ssim_values:
                    f.write(
                        f"- **Average SSIM:** {np.mean(ssim_values):.4f} (range: {min(ssim_values):.4f} - {max(ssim_values):.4f})\n"
                    )
                f.write(
                    f"- **Average MAE:** {np.mean(mae_values):.4f} (range: {min(mae_values):.4f} - {max(mae_values):.4f})\n\n"
                )
