...
class MaterialDetector:
    ...
    def detect(self, image_path: Path) -> MaterialDetectionResult:
        ...
        for material_type in MaterialType:
            confidence_map = self._detect_material(hsv_img, texture_map, specular_map, material_type)
        ...
        dominant_material = max(materials.items(), key=lambda x: x[1].percentage)[0] if materials else MaterialType.STONE
    ...
    def generate_heatmap(
        ...
        draw.rectangle([(10, 10), (20 + text_width, 20 + text_height)], fill=(0, 0, 0, 180))
    ...
    def enhance_with_confidence(
        ...
        for material_type, confidence_data in result.materials.items():
            ...
            material_enhanced = enhancement_func(image, material_type, base_strength * confidence_data.mean_confidence)
...
def main():
    ...
    sorted_materials = sorted(result.materials.items(), key=lambda x: x[1].percentage, reverse=True)
    ...
    if args.generate_heatmaps:
        print(f"\n🎨 Generating heatmaps...")
        materials_to_viz = [MaterialType(m) for m in args.materials] if args.materials else result.materials.keys()
