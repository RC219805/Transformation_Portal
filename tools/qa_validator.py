...
class QAValidator:
    ...
    def validate_image(self, path: Path) -> ImageValidation:
        ...
        has_critical_warnings = (
            any(issue.severity == "warning" and issue.category in ["corruption", "bit_depth"] for issue in issues)
            if self.strict_mode
            else False
        )
    ...
    def generate_report(self, output_path: Path, summary: Dict[str, Any]):
        ...
        with open(output_path, "w") as f:
            ...
            for validation in self.validations:
                ...
                f.write(
                    f"- **Resolution:** {meta.get('width', 0)}x{meta.get('height', 0)} ({meta.get('megapixels', 0):.1f} MP)\n"
                )
