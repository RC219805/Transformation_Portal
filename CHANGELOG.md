...
### Added
- **LuxDepthV5 opt-in photography pipeline:** Added `lux-depth-v5`, Execution Plan V4, depth validity/sky evidence, explicit inference precision, conservative guided reconstruction, and reference-bound quality evaluation. Package metadata advances to 0.6.0 for the additive console entrypoint; V3/V4 commands remain available. Production depth-quality and performance acceptance remain pending.
...
### Fixed
- **Depth inference contracts:** Preserve floating DA2 predictions and source-grid geometry; honor Depth Pro checkpoint and focal metadata; reject incompatible metric/relative ensemble fusion and exclude synthetic contributors from genuine statistics. Cache recipes invalidate preceding adapter outputs.
- **Dependency security alert closure:** Raised the supported ML and isolated runtime baselines to Pillow 12.3.0, torch 2.13.0 with torchvision 0.28.0, and Transformers 5.5.x; moved the frontdoor to Next 16.3.0 with sharp 0.35.3 and js-yaml 4.3.1; and forced the root/Worker Wrangler trees onto undici 7.29.0. The Vercel build now delegates deployment output to its adapter while local builds retain standalone output.
- **Code scanning alert closure:** Route every CAS digest through the centralized SHA-256 path-safety helpers before object or lock path construction, preserve dashboard 404 contracts for invalid digests, and redact FastVLM validation errors and filesystem paths from both human and JSON output.
- **Fail-closed dependency audit:** Parse pip-audit 2.10.1's nested advisory schema, propagate scanner/report failures, block every known advisory, and pin the scanner environment to non-vulnerable setuptools 83.0.0.
