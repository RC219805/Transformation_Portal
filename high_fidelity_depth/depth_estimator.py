...
class HighFidelityDepthEstimator:
    ...
    def _compute_target_size(self, height: int, width: int, input_size: int) -> Tuple[int, int]:
        """
    ...
    def _pad_to_patch_multiple(self, image: np.ndarray) -> Tuple[np.ndarray, Tuple[int, int]]:
        """
        ...
        padded = cv2.copyMakeBorder(image, 0, pad_h, 0, pad_w, borderType=cv2.BORDER_REFLECT_101)

        logger.debug(f"Padded {h}×{w} → {padded.shape[0]}×{padded.shape[1]} (patch multiple, {pad_h}×{pad_w} padding)")

        return padded, (pad_h, pad_w)

    def preprocess_for_inference(self, image: np.ndarray, force_input_size: Optional[int] = None) -> Tuple[np.ndarray, dict]:
        """
        ...
        target_h, target_w = self._compute_target_size(original_h, original_w, input_size)
    ...
    def postprocess_depth(self, depth: np.ndarray, metadata: dict) -> np.ndarray:
        """
        ...
        logger.debug(f"Postprocessing: {metadata['preprocessed_shape']} → {original_h}×{original_w}")
    ...
    def _pad_to_tile_geometry(self, image: np.ndarray) -> Tuple[np.ndarray, Tuple[int, int, int, int]]:
        ...
        padded = cv2.copyMakeBorder(image, top, bottom, left, right, borderType=cv2.BORDER_REFLECT_101)
    ...
    def _infer_tile_depth(self, tile_rgb: np.ndarray, force_input_size: Optional[int] = None) -> np.ndarray:
        ...
        logger.info(
            f"🔍 Tile inference: preprocessed={preprocessed.shape[0]}×{preprocessed.shape[1]}, "
    ...
    def _smooth_tile_calibrations(
        self, calibrations_grid: Dict[Tuple[int, int], Tuple[float, float]]
    ...
    def _blend_tiles_with_reconciliation(
        ...
        for idx, (tile_depth, y0, y1, x0, x1) in enumerate(tile_depths):
            ...
            if self.config.reconcile_scales and (overlap_mask.sum() > 0 or global_anchor is not None):
                tile_calibrated, a, b = self._reconcile_tile_scale(tile_depth, reference_region, overlap_mask)
    ...
    def estimate_depth(
        ...
        if min(h, w) < self.config.small_image_threshold:
            ...
            try:
                inputs = self.image_processor(images=image_pil, return_tensors="pt", do_resize=False, do_pad=False)
            except TypeError:
            ...
            logger.info(
                f"🔍 Direct inference: preprocessed={preprocessed.shape[0]}×{preprocessed.shape[1]}, "
