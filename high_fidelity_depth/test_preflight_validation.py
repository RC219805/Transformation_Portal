...
class TestStrideConsistency:
    ...
    def test_stride_calculation(self):
        ...
        for size in [2000, 3000, 4000, 5000]:
            ...
            assert stride == expected_stride, f"Stride mismatch: {stride} != {expected_stride}"
            ...
            assert last_tile_end_h == h_pad, f"Vertical coverage gap: {last_tile_end_h} != {h_pad}"
            assert last_tile_end_w == w_pad, f"Horizontal coverage gap: {last_tile_end_w} != {w_pad}"
