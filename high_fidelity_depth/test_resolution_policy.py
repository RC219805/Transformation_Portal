...
def test_patch_multiple_computation():
    ...
    for (h, w), input_size, expected in test_cases:
        ...
        assert actual_short == expected_short, \
            f"Shortest side should be {expected_short}, got {actual_short}"
...
def test_aspect_ratio_preserved():
    ...
    assert aspect_error < 0.05, \
        f"Aspect ratio error {aspect_error:.3f} exceeds 5% tolerance"
...
def test_roundtrip_dimension_preservation():
    ...
    for h, w in test_sizes:
        ...
        assert depth_restored.shape == (h, w), \
