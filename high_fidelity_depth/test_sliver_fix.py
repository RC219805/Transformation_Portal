...
def test_dimension(h: int, w: int, pattern: str = "gradient") -> dict:
    ...
    config = DepthConfig(tile_size=1024, overlap=192, validate_seams=True, seam_energy_threshold=1.2)
    ...
    try:
        ...
        grad_mag = np.sqrt(cv2.Sobel(depth, cv2.CV_32F, 1, 0, ksize=3) ** 2 + cv2.Sobel(depth, cv2.CV_32F, 0, 1, ksize=3) ** 2)
