...
class EdgeMetrics:
    ...
    def passed(self, strict: bool = False) -> bool:
        ...
        if strict:
            return (
                self.edge_f1 >= 0.45
                and self.edge_overlap >= 0.50
                and self.edge_count_ratio <= 2.0
                and self.halo_score >= 0.7
                and self.overshoot_penalty <= 0.3
            )
        else:
            return (
                self.edge_f1 >= 0.30
                and self.edge_overlap >= 0.40
                and self.edge_count_ratio <= 3.0
                and self.overshoot_penalty <= 0.5
            )
...
def compute_edge_f1(rgb_edges: np.ndarray, depth_edges: np.ndarray, tolerance: int = 2) -> float:
    """
...
def compute_edge_overlap(rgb_edges: np.ndarray, depth_edges: np.ndarray, dilation: int = 3) -> float:
    """
...
def compute_chamfer_distance(rgb_edges: np.ndarray, depth_edges: np.ndarray) -> float:
    """
...
def compute_edge_alignment_corr(rgb_edges: np.ndarray, depth_edges: np.ndarray) -> float:
    """
...
def detect_halos(depth: np.ndarray, rgb_edges: np.ndarray) -> float:
    ...
    logger.debug(
        f"Halo detection: edge_overshoot={edge_overshoot:.6f}, "
...
def compute_overshoot_penalty(depth: np.ndarray) -> float:
    ...
    logger.info(
        f"Overshoot penalty: raw_p95={penalty_raw:.4f}, penalty={penalty:.3f}, "
...
def compute_high_frequency_energy(depth_map: np.ndarray, sigma: float = 15.0) -> float:
    ...
    logger.debug(f"HF energy: {hf_energy:.6f} (sigma={sigma:.1f}, global_var={np.var(depth_map):.6f})")
...
def save_metrics_atomic(metrics: Dict, output_path: Path) -> None:
    ...
    # Write to temp file
    temp_fd, temp_path = tempfile.mkstemp(suffix=".json", dir=output_path.parent, text=True)
