...
class LuxuryEstateMasterPipeline:
    ...
    def _init_depth_model(self):
        ...
        try:
            ...
            self.depth_model = DepthAnythingV2Model(
                variant=variant_map[self.preset.depth.model_variant],
                backend=backend_map.get(self.preset.depth.backend),
                precision='fp16',
            )
            logger.info(f"Loaded Depth Anything V2 ({self.preset.depth.model_variant}, {self.preset.depth.backend})")
        except Exception as e:
    ...
    def _stage_2_depth_estimation(self, image_linear: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            ...
            # Estimate depth
            depth_map = self.depth_model.estimate(image_uint8)
    ...
    def _stage_6_ai_enhancement(self, image: np.ndarray, depth_map: Optional[np.ndarray], 
        ...
        try:
            ...
            if cfg.ai_enhancement_padding:
                ...
                # Convert back to PIL
                image_resized = Image.fromarray((np.clip(image_np, 0, 1) * 255).astype(np.uint8))
            
            # Generate Canny edges
            canny = self.canny_detector(image_resized, 100, 200)
    ...
    def _stage_7_upscaling(self, image: np.ndarray, original_size: Tuple[int, int]) -> np.ndarray:
        ...
        if self.upscaler is None:
            # Fallback to Lanczos
            logger.info("  → Using Lanczos (ESRGAN not available)")
            image_pil = Image.fromarray((np.clip(image, 0, 1) * 255).astype(np.uint8))
