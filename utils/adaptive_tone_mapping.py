...
class AdaptiveToneMapper:
    ...
    def analyze_scene(self, hdr_image: np.ndarray) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "color_stats": {"avg_saturation": float(avg_saturation)},
            "scene_classification": scene_type,
            "recommended_params": {"key": key, "sat": sat, "epsilon": 1e-6},
            "reasoning": self._generate_reasoning(scene_type, key, sat, dynamic_range),
        }

        return analysis

    def _classify_scene_brightness(self, lum_avg: float, lum_median: float, peak_value: float) -> str:
        """
    ...
    def _determine_key_value(self, scene_type: str, lum_avg: float, lum_median: float, dynamic_range: float) -> float:
        """
    ...
    def _generate_reasoning(self, scene_type: str, key: float, sat: float, dynamic_range: float) -> str:
    ...
    def apply_adaptive_tone_mapping(
        self, hdr_image: np.ndarray, override_params: Dict[str, float] = None
    ...
    def _reinhard_local(self, hdr_image: np.ndarray, key: float, sat: float, epsilon: float) -> np.ndarray:
