...
class DepthProcessor:
    ...
    def apply_zone_adjustments(self, image: np.ndarray, depth_map: np.ndarray) -> np.ndarray:
        """
        ...
        if self.config.foreground_boost != 1.0:
            ...
            enhanced = enhanced * (1 - foreground[:, :, None]) + (enhanced ** (1 / fg_adjustment)) * foreground[:, :, None]
    ...
    def save_depth_visualization(self, depth_map: np.ndarray, output_path: Path, colormap: str = "viridis"):
