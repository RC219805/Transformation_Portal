...
class LUTProcessor:
    ...
    def _load_lut(self, lut_path: Path):
        ...
        if self.is_3d:
            ...
            if len(lut_array) != expected_size:
                raise ValueError(f"3D LUT size mismatch: expected {expected_size}, got {len(lut_array)}")
            # Reshape to 3D grid
            self.lut_data = lut_array.reshape((lut_size, lut_size, lut_size, 3))
        else:
            if len(lut_array) != lut_size:
                raise ValueError(f"1D LUT size mismatch: expected {lut_size}, got {len(lut_array)}")
        ...
        logger.info(f"Loaded {'3D' if self.is_3d else '1D'} LUT: size={lut_size}, title={self.title}")

    def apply(self, image: np.ndarray, strength: Optional[float] = None) -> np.ndarray:
        """
        Apply LUT to image.
    ...
    def _apply_1d_lut(self, image: np.ndarray) -> np.ndarray:
        ...
        for ch in range(3):
            ...
            # Interpolate
            result[:, :, ch] = self.lut_data[indices_floor, ch] * (1 - weight) + self.lut_data[indices_ceil, ch] * weight

        return result

    def _preserve_highlights(self, original: np.ndarray, processed: np.ndarray, threshold: float = 0.9) -> np.ndarray:
        """
    ...
    def _preserve_blacks(self, original: np.ndarray, processed: np.ndarray, threshold: float = 0.1) -> np.ndarray:
        """
