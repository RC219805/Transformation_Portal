...
class ExposureFusion:
    ...
    def extract_brackets(
        self, hdr_image: np.ndarray, num_brackets: int = 3, ev_range: float = 2.0
    ...
    def _tone_map(self, hdr: np.ndarray, method: str = "reinhard") -> np.ndarray:
        ...
        if method == "reinhard":
            ...
            # Scale RGB channels
            scale = np.divide(lum_mapped, lum + 1e-8, where=lum > 0)[..., np.newaxis]
    ...
    def fuse_exposures(self, brackets: List[np.ndarray], method: str = "laplacian") -> np.ndarray:
        """
