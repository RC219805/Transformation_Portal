...
class PerformanceProfiler:
    ...
    def _identify_bottlenecks(self) -> List[str]:
        ...
        for stage in self.stages:
            ...
            if mem_increase > 1000:  # > 1GB
                bottlenecks.append(f"Stage '{stage.name}' uses significant memory (+{mem_increase:.0f}MB peak increase)")
        ...
        for stage in self.stages:
            if stage.throughput is not None and stage.throughput < 1.0:
                bottlenecks.append(
                    f"Stage '{stage.name}' has low throughput ({stage.throughput:.2f} items/sec) - consider parallelization"
                )
    ...
    def print_report(self, report: PerformanceReport):
        ...
        for stage in report.stages:
            ...
            print(f"{stage.name:<30} {stage.duration:>8.2f}s  {mem_str:<15} {throughput_str:<15}")
