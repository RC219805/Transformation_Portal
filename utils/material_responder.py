...
class MaterialResponder:
    ...
    def detect_materials(self, image: np.ndarray) -> Dict[SurfaceType, np.ndarray]:
        ...
        for surface_type in material_maps:
            material_maps[surface_type] = gaussian_filter(material_maps[surface_type], sigma=3)
    ...
    def enhance_surface(
        ...
        if "contrast_boost" in profile:
            ...
            enhanced = enhanced * (1 - effective_confidence[:, :, None]) + contrast_enhanced * effective_confidence[:, :, None]
        ...
        for surface_name in surfaces:
            ...
            if surface_type in material_maps:
                ...
                if confidence_map.max() > 0.1:
                    logger.info(f"  Enhancing {surface_type.value} (coverage: {confidence_map.mean() * 100:.1f}%)")
                    enhanced = self.enhance_surface(enhanced, surface_type, confidence_map, depth_map)
...
def create_material_responder(
    ...
    """
    config = MaterialResponseConfig(strength=strength, surface_types=surfaces, depth_aware=depth_aware)
