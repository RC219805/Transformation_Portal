// web/secure-landing/portal-src/review-surface-deferred.js
function createDeferredReviewSurfaceApi(host) {
  const {
    state,
    els,
    clamp,
  ...
  const ARTIFACT_VIEWER_FETCH_TIMEOUT_MS = 15e3;
  ...
  function _showArtifactViewerFallback(context, artifactName, fallbackOptions) {
    ...
    const fallbackReason = isRetryable ? "inline_preview_failed" : url ? "inline_preview_unavailable" : "asset_url_unavailable";
    ...
    if (els.artifactViewerFallbackTitle) {
      els.artifactViewerFallbackTitle.textContent = isRetryable ? "Inline preview failed to load" : url ? "Inline preview unavailable" : "Artifact URL unavailable";
    }
    if (els.artifactViewerFallbackDetail) {
      els.artifactViewerFallbackDetail.textContent = isRetryable ? "The managed asset request did not complete. Retry the preview or continue reviewing the retained metadata and fingerprints." : url ? "This artifact stays reviewable through retained metadata, integrity fingerprints, and the managed raw asset link." : "The browser cannot resolve a managed asset URL for this artifact, so review stays pinned to the retained metadata above.";
    }
    _renderArtifactViewerRetry(isRetryable ? { context, artifactName } : null);
    _setArtifactViewerStatus(
      isRetryable ? `${artifactName} inline preview failed to load; retry available.` : url ? `${artifactName} is open with metadata fallback because an inline preview is unavailable.` : `${artifactName} is open with metadata fallback because the managed asset URL is unavailable.`
    );
  ...
  async function _loadArtifactViewerInlinePreview(context, artifactName) {
    ...
    }, ARTIFACT_VIEWER_FETCH_TIMEOUT_MS) : null;
    try {
      if (state.auth?.mode === "direct_debug" && typeof _buildAuthHeaders === "function") {
        ...
        if (requestId !== artifactViewerPreviewRequestId || !state.portalUi?.artifactViewer?.open || state.portalUi.artifactViewer.artifactPath !== artifactPath) {
      ...
      if (!retryable) return;
      const failureMessage = abortReason === "timeout" ? `request timed out after ${ARTIFACT_VIEWER_FETCH_TIMEOUT_MS / 1e3}s` : err?.message || "network error";
      try {
  ...
  function _captionStemFromArtifact(artifact) {
    ...
    return fileName.replace(/\.vlm_captioning\.sidecar\.json$/i, "").replace(/\.vlm_captioning\.raw\.txt$/i, "").replace(/_proxy\.(png|jpe?g)$/i, "");
  }
  ...
  function _renderArtifactMetadataCard(job, artifact) {
    ...
    const title = document.createElement("p");
    title.className = "text-[12px] font-semibold text-slate-800 dark:text-slate-100";
    title.textContent = artifact ? artifactLabel(artifact) : "Select a completed run to bring the primary review artifact into focus here.";
  ...
  function _renderArtifactProvenance(job, artifact) {
    ...
    const artifactDescriptor = artifact ? `${artifactDisplayLabel(artifact)} \u2022 ${artifactContentType(artifact) || "binary"} \u2022 ${formatBytes(artifact.size_bytes)}` : "Awaiting indexed artifact";
  ...
  function renderArtifactPanel() {
    ...
    if (els.artifactSelectionMeta) {
      els.artifactSelectionMeta.textContent = selectedArtifact ? `${artifactDisplayLabel(selectedArtifact)} \u2022 ${artifactContentType(selectedArtifact) || "binary"} \u2022 ${formatBytes(selectedArtifact.size_bytes)}` : "Preview, metadata, and actions will appear here when outputs are indexed.";
    }
    ...
    artifacts.forEach((artifact) => {
      ...
      button.className = active ? "rounded-2xl border border-cyan-300 dark:border-cyan-900/60 bg-cyan-50/90 dark:bg-cyan-900/20 p-3 text-left shadow-sm transition-colors" : "rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/50 p-3 text-left hover:bg-white/90 dark:hover:bg-slate-800/80 transition-colors";
  ...
  function _artifactViewerContext() {
    ...
    const artifact = artifacts.find((candidate) => _artifactRouteKey(candidate) === requestedPath) || _selectedArtifactForJob(job) || artifacts[0] || null;
...
export {
  createDeferredReviewSurfaceApi
};
