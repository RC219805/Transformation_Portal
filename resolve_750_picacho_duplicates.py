...
class DuplicateResolver:
    ...
    def generate_processing_list(self, manifest: Dict) -> List[Path]:
        ...
        return [
            Path(info['path'])
            for scene, info in sorted(manifest['canonical_sources'].items())
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Resolve duplicate file issues in 750 Picacho project'
    )
    parser.add_argument(
        'base_dir',
        type=Path,
        help='Base directory containing 750 Picacho files'
    )
    parser.add_argument(
        '--cleanup',
        action='store_true',
