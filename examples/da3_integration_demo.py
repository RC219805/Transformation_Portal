...
def example_2_advanced_processing():
    ...
    estimator = DA3DepthEstimator(model="large-1.1", device="cpu", verbose=True, auto_cleanup=True)
...
def example_5_model_comparison():
    ...
    for model_name in models_to_test:
        ...
        if result.success and result.depth_array is not None:
            depth = result.depth_array
            print(f"   ✅ {model_name}: Shape={depth.shape}, Range=[{depth.min():.3f}, {depth.max():.3f}]")
        else:
...
def example_6_pipeline_integration():
    ...
    class MockRenderingPipeline:
        ...
        def __init__(self):
            self.depth_estimator = DA3DepthEstimator(model="large-1.1", device="cpu")
        ...
        def process_property(self, image_path, output_dir):
            ...
            depth_result = self.depth_estimator.process_image(
                image_path, output_dir / "depth", export_format="mini_npz-depth_vis"
            )
